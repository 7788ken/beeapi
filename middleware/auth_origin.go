...
// SessionCookieOriginGuard protects browser authentication and refresh-cookie
// endpoints when secure cookie mode is enabled. It does not add CORS headers
// and must not be installed on API-key relay routes.
func SessionCookieOriginGuard() gin.HandlerFunc {
	return func(c *gin.Context) {
		if !requireSessionCookieOrigin(c) {
			return
		}
		c.Next()
	}
...
func requireSessionCookieOrigin(c *gin.Context) bool {
