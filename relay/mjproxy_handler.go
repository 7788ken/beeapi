...
func RelaySwapFace(c *gin.Context, info *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	userQuota, err := model.GetUserQuota(info.UserId)
	if err != nil {
		return &dto.MidjourneyResponse{
			Code:        4,
			Description: err.Error(),
	...
	defer func() {
		if mjResp.StatusCode == 200 && mjResp.Response.Code == 1 && midjourneyTask != nil && midjourneyTask.Id > 0 {
			err := service.ConsumeMidjourneyQuota(midjourneyTask, info, priceData.Quota, true)
			if err != nil {
			...
			if err := model.UpdateUserAndChannelUsedQuota(info.UserId, info.ChannelId, priceData.Quota); err != nil {
				common.SysError("failed to update midjourney usage statistics: " + err.Error())
			}
		}
	}()
...
func RelayMidjourneySubmit(c *gin.Context, relayInfo *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	defer func() {
		if consumeQuota && midjResponseWithStatus.StatusCode == 200 && midjourneyTask != nil && midjourneyTask.Id > 0 {
			err := service.ConsumeMidjourneyQuota(midjourneyTask, relayInfo, priceData.Quota, true)
			if err != nil {
				common.SysLog("error consuming token remain quota: " + err.Error())
				return
			}
			...
			if err := model.UpdateUserAndChannelUsedQuota(relayInfo.UserId, relayInfo.ChannelId, priceData.Quota); err != nil {
				common.SysError("failed to update midjourney usage statistics: " + err.Error())
			}
		}
	}()
