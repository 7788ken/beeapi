...
func RelayTaskSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*TaskSubmitResult, *dto.TaskError) {
	...
	if !common.StringsContains(constant.TaskPricePatches, modelName) {
		...
		quota, err := common.QuotaFromFloatStrict(quotaWithRatios)
		if err != nil {
			return nil, service.TaskErrorWrapper(err, "model_price_error", http.StatusBadRequest)
		}
	...
	// 8. 构建请求体；开启 PassThroughBody 时原样转发客户端 body（对齐 image/claude/gemini 等 handler）
	var requestBody io.Reader
	if model_setting.GetGlobalSettings().PassThroughRequestEnabled ||
		info.ChannelSetting.PassThroughBodyEnabled {
		storage, storageErr := common.GetBodyStorage(c)
		if storageErr != nil {
			return nil, service.TaskErrorWrapper(storageErr, "read_body_failed", http.StatusBadRequest)
		}
	...
	if adjustedRatios := adaptor.AdjustBillingOnSubmit(info, taskData); len(adjustedRatios) > 0 {
		...
		finalQuota, clamp = recalcQuotaFromRatios(info, adjustedRatios)
...
func recalcQuotaFromRatios(info *relaycommon.RelayInfo, ratios map[string]float64) (int, *common.QuotaClamp) {
