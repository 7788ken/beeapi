...
func TestImageRequestTokenMetaIncludesRequestedCount(t *testing.T) {
	n := uint(3)
	meta := (&dto.ImageRequest{Model: "dall-e-3", Size: "1024x1024", N: &n}).GetTokenCountMeta()
	require.Equal(t, 3.0, meta.BillingRatios["n"])

	meta = (&dto.ImageRequest{Model: "dall-e-3"}).GetTokenCountMeta()
	require.Equal(t, 1.0, meta.BillingRatios["n"])
}
