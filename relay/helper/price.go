...
func modelPriceNotConfiguredError(c *gin.Context, info *relaycommon.RelayInfo, modelName string) error {
	// 审计：记录被拦截请求，便于发现攻击尝试 + 统计简写覆盖率（哪些 model 频繁打到该路径）
	logger.LogWarn(c, fmt.Sprintf(
		"unpriced model intercepted: user_id=%d role=%d model=%s setting_accept_unset=%v",
		info.UserId, info.UserRole, modelName, info.UserSetting.AcceptUnsetRatioModel,
	))
...
// Used when a paid tiered request omits max_tokens, so pre-consume still
// reserves a plausible output cost instead of prompt-only quota.
...
func ModelPriceHelper(c *gin.Context, info *relaycommon.RelayInfo, promptTokens int, meta *types.TokenCountMeta) (types.PriceData, error) {
	...
	if !usePrice {
		...
		if !success {
			...
			// 双保险：仅管理员的 AcceptUnsetRatioModel 才生效，防止 setting 残留或越权写入被利用
			if info.UserSetting.AcceptUnsetRatioModel && info.UserRole >= common.RoleAdminUser {
				acceptUnsetRatio = true
			}
			if !acceptUnsetRatio {
				return types.PriceData{}, modelPriceNotConfiguredError(c, info, matchName)
			}
		}
...
func ModelPriceHelperPerCall(c *gin.Context, info *relaycommon.RelayInfo) (types.PriceData, error) {
	...
	if !success {
		...
		if ok {
			...
			acceptUnsetRatio := false
			// 双保险：仅管理员的 AcceptUnsetRatioModel 才生效
			if info.UserSetting.AcceptUnsetRatioModel && info.UserRole >= common.RoleAdminUser {
				acceptUnsetRatio = true
			}
