...
func TestModelPriceHelperTieredUsesDefaultOutputEstimate(t *testing.T) {
	gin.SetMode(gin.TestMode)
	saved := map[string]string{}
	require.NoError(t, config.GlobalConfig.SaveToDB(func(key, value string) error {
		saved[key] = value
...
func TestModelPriceHelperRejectsOverflowAndPreConsumesImageCount(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	ctx, info := newContext("fixed-image-price")
	priceData, err := ModelPriceHelper(ctx, info, 1000, &types.TokenCountMeta{
		ImagePriceRatio: 3,
	...
	ctx, info = newContext("overflow-image-price")
