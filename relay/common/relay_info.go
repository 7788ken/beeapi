...
type RelayInfo struct {
	...
	TokenGroup        string
	UserId            int
	UserRole          int    // 用户角色，由 token-auth 中间件经 UserBase.WriteContext 写入
	...
	// QuotaClamp records a final-settlement saturation event for admin audit.
	// Pre-consume conversions fail fast and do not populate this field.
...
type TaskSubmitReq struct {
	...
	// Content 为上游原生多模态数组（每项含 type=text/image_url/video_url/audio_url + role），
	// 由支持多模态的渠道（如 doubao/seedance）受控透传给上游；不支持多模态的渠道忽略此字段。
	Content []map[string]interface{} `json:"content,omitempty"`
	// Videos 便捷视频参考字段（视频 URL 或 asset://），对称 Images；
	// 由 doubao/seedance 转成 content[{type:video_url, role:reference_video}]。
...
func hasRemovableDisabledField(jsonData []byte, channelOtherSettings dto.ChannelOtherSettings) bool {
	...
	if !channelOtherSettings.AllowSpeed && gjson.GetBytes(jsonData, "speed").Exists() {
		return true
	}
	...
	return !channelOtherSettings.AllowIncludeObfuscation && gjson.GetBytes(jsonData, "stream_options.include_obfuscation").Exists()
}
