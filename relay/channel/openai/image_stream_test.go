...
func TestOpenaiImageStreamHandlerClientDisconnectDrainsFinalUsage(t *testing.T) {
	oldMode := gin.Mode()
	gin.SetMode(gin.TestMode)
	t.Cleanup(func() { gin.SetMode(oldMode) })
	oldTimeout := constant.StreamingTimeout
	...
	body := strings.Join([]string{
		`data: {"type":"image_generation.completed","b64_json":"first"}`,
		``,
		`data: {"type":"image_generation.completed","b64_json":"second","usage":{"input_tokens":3,"output_tokens":4,"total_tokens":7}}`,
		``,
		`data: [DONE]`,
		``,
	}, "\n")
	c, _, resp, info := newDisconnectingImageStream(t, body, "first")
	info.PriceData.UsePrice = true
	...
	require.Equal(t, 2.0, info.PriceData.OtherRatios["n"])
	require.Contains(t,
		[]relaycommon.StreamEndReason{relaycommon.StreamEndReasonClientGone, relaycommon.StreamEndReasonDone, relaycommon.StreamEndReasonEOF},
...
func TestOpenaiImageStreamHandlerRecordsUpstreamErrorEvent(t *testing.T) {
	...
	require.Contains(t, info.StreamStatus.ErrorMessages()[0], "INTERNAL_ERROR")
