...
type AliVideoInput struct {
	...
	Media          []AliVideoMedia `json:"media,omitempty"`           // Wan2.7 image-to-video protocol
...
func normalizeWan27I2VInput(aliReq *AliVideoRequest, req relaycommon.TaskSubmitReq) error {
	...
	if len(aliReq.Input.Media) == 0 {
		...
		if aliReq.Input.AudioURL != "" {
			aliReq.Input.Media = append(aliReq.Input.Media, AliVideoMedia{Type: "driving_audio", URL: aliReq.Input.AudioURL})
		}
	}
	if len(aliReq.Input.Media) == 0 {
...
func (a *TaskAdaptor) convertToAliRequest(info *relaycommon.RelayInfo, req relaycommon.TaskSubmitReq) (*AliVideoRequest, error) {
	...
	// metadata 里的 parameters.duration 会覆盖上面校验过的时长，绕过标准
	// duration/seconds 字段校验。若不在此拒绝，越界值会在计费层被静默钳到
	// 上限并按顶格时长扣费，与 gemini/veo 的 metadata 校验保持一致语义。
...
func (a *TaskAdaptor) EstimateBilling(c *gin.Context, info *relaycommon.RelayInfo) map[string]float64 {
	...
	// metadata 可绕过标准请求校验直接改写 Duration；作为计费乘数前必须钳制上限。
