...
// MIME 白名单已改为运行时配置，见 setting/model_setting/gemini.go 的 SupportedMimeTypes
	...
	for _, message := range textRequest.Messages {
		...
		for _, part := range openaiContent {
			if part.Type == dto.ContentTypeText {
				...
				// 校验 MimeType 是否在 Gemini 支持的白名单中（后台可配置，空数组则不校验）
				if !model_setting.IsGeminiSupportedMimeType(mimeType) {
					return nil, fmt.Errorf("mime type is not supported by Gemini: '%s', url: '%s', supported types are: %v", mimeType, source.GetIdentifier(), model_setting.GetGeminiSupportedMimeTypes())
				}
...
func FetchGeminiModels(parent context.Context, baseURL, apiKey, proxyURL string) ([]string, error) {
