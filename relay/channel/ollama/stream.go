...
func ollamaToolCallsToOpenAI(toolCalls []OllamaToolCall, startIndex int, includeIndex bool) ([]dto.ToolCallResponse, int) {
	...
	for _, tc := range toolCalls {
		// fork 加固（超出上游 0977965d9）：上游未校验 function.name。
		// 空名工具调用对任何客户端都不可用（OpenAI SDK 把 function.name 当必填），
		// 直接照搬会把原本的「静默丢弃」升级成客户端侧 SDK 校验失败，
		// 且 finish_reason 会被翻成 tool_calls。跳过后本条退回上游未修前的行为，
		// 与 BE-41 在 OpenAI→Claude 请求侧加的 name=="" 守卫保持同一准入标准。
		// 上游同步时注意保留。
		if tc.Function.Name == "" {
			continue
		}
