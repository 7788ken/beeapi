...
func TestBuildOpenAIStyleUsageExposesCacheWriteTokens(t *testing.T) {
	usage := buildOpenAIStyleUsageFromClaudeUsage(&dto.Usage{
		PromptTokens:     100,
		CompletionTokens: 20,
	...
	require.Equal(t, 50, usage.PromptTokensDetails.CacheWriteTokens)
...
func TestStreamResponseClaude2OpenAIUsesNativeContentBlockIndex(t *testing.T) {
	name := "lookup"
	firstIndex := 0
	firstResponse := StreamResponseClaude2OpenAI(&dto.ClaudeResponse{
	...
	require.Len(t, firstResponse.Choices, 1)
	require.Len(t, firstResponse.Choices[0].Delta.ToolCalls, 1)
	require.NotNil(t, firstResponse.Choices[0].Delta.ToolCalls[0].Index)
	require.Equal(t, 0, *firstResponse.Choices[0].Delta.ToolCalls[0].Index)

	partialJSON := `{"city":"Tokyo"}`
	secondIndex := 1
	secondResponse := StreamResponseClaude2OpenAI(&dto.ClaudeResponse{
		Type:  "content_block_delta",
		Index: &secondIndex,
		Delta: &dto.ClaudeMediaMessage{
	...
	require.Len(t, secondResponse.Choices, 1)
	require.Len(t, secondResponse.Choices[0].Delta.ToolCalls, 1)
	require.NotNil(t, secondResponse.Choices[0].Delta.ToolCalls[0].Index)
	require.Equal(t, 1, *secondResponse.Choices[0].Delta.ToolCalls[0].Index)
	require.Equal(t, partialJSON, secondResponse.Choices[0].Delta.ToolCalls[0].Function.Arguments)
}

func TestStreamResponseClaude2OpenAIMapsToolIndexAfterTextBlock(t *testing.T) {
	claudeInfo := &ClaudeResponseInfo{}
	toolBlockIndex := 1
	startResponse := streamResponseClaude2OpenAI(&dto.ClaudeResponse{
		Type:  "content_block_start",
	...
	require.Len(t, startResponse.Choices, 1)
	require.Len(t, startResponse.Choices[0].Delta.ToolCalls, 1)
	require.NotNil(t, startResponse.Choices[0].Delta.ToolCalls[0].Index)
	require.Equal(t, 0, *startResponse.Choices[0].Delta.ToolCalls[0].Index)

	partialJSON := `{"city":"Tokyo"}`
	deltaResponse := streamResponseClaude2OpenAI(&dto.ClaudeResponse{
		Type:  "content_block_delta",
		Index: &toolBlockIndex,
		Delta: &dto.ClaudeMediaMessage{
			Type:        "input_json_delta",
	...
	require.Len(t, deltaResponse.Choices, 1)
	require.Len(t, deltaResponse.Choices[0].Delta.ToolCalls, 1)
	require.NotNil(t, deltaResponse.Choices[0].Delta.ToolCalls[0].Index)
	require.Equal(t, 0, *deltaResponse.Choices[0].Delta.ToolCalls[0].Index)
	require.Equal(t, partialJSON, deltaResponse.Choices[0].Delta.ToolCalls[0].Function.Arguments)
}
