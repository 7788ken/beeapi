...
func RequestOpenAI2ClaudeMessage(c *gin.Context, textRequest dto.GeneralOpenAIRequest) (*dto.ClaudeRequest, error) {
	result, err := relayconvert.ConvertRequest(c, nil, types.RelayFormatClaude, &textRequest)
	if err != nil {
		return nil, err
	}
	request, ok := result.Value.(*dto.ClaudeRequest)
	if !ok {
...
func streamResponseClaude2OpenAI(claudeResponse *dto.ClaudeResponse, claudeInfo *ClaudeResponseInfo) *dto.ChatCompletionsStreamResponse {
	return relayconvert.StreamResponseClaude2OpenAIWithInfo(claudeResponse, claudeInfo)
}
