...
type waffoPancakeWebhookData struct {
	...
	// Waffo 实际 webhook 把 createSession 时传入的 metadata 放在 data.orderMetadata。
	// 同时保留 data.metadata 兼容（不同 payload 版本 / 早期文档措辞）。
...
type waffoPancakeWebhookEvent struct {
	...
	// 兜底：万一 metadata 不在 data 里而在外层（不同 webhook payload 版本）。
	Metadata map[string]string `json:"metadata,omitempty"`
}

// metadata 通道里我们存放 trade_no 用的 key。
const waffoPancakeMetadataTradeNoKey = "tradeNo"

// extractTradeNoFromMetadata 依次在 data.orderMetadata、data.metadata、外层 metadata 三处查找。
// 实测 Waffo 用 data.orderMetadata；其余位置作为兼容兜底。
func extractTradeNoFromMetadata(event *waffoPancakeWebhookEvent) string {
	if event == nil {
		return ""
	}
	if v, ok := event.Data.OrderMetadata[waffoPancakeMetadataTradeNoKey]; ok {
		if t := strings.TrimSpace(v); t != "" {
			return t
		}
	}
	if v, ok := event.Data.Metadata[waffoPancakeMetadataTradeNoKey]; ok {
		if t := strings.TrimSpace(v); t != "" {
			return t
		}
	}
	if v, ok := event.Metadata[waffoPancakeMetadataTradeNoKey]; ok {
		if t := strings.TrimSpace(v); t != "" {
			return t
		}
	}
...
func ResolveWaffoPancakeTradeNo(event *waffoPancakeWebhookEvent) (string, error) {
	...
	// 主路径：metadata 通道。createSession 时塞入 {"tradeNo": tradeNo}，期望 webhook 原样回传。
	if tradeNo := extractTradeNoFromMetadata(event); tradeNo != "" {
	...
	// 备路径：Waffo orderId ↔ TopUp.ProviderOrderID 反查（依赖 createSession 是否返回 orderId；
	// 当前实测响应里为空，留作日后上游若补字段时即时可用）。
	if orderID := strings.TrimSpace(event.Data.OrderID); orderID != "" {
		if topUp := model.GetTopUpByProviderOrderID(model.PaymentProviderWaffoPancake, orderID); topUp != nil {
			return topUp.TradeNo, nil
		}
	}

	// 兜底：旧伪邮箱通道，覆盖切换前已发出但仍 pending 的订单。
	if email := strings.TrimSpace(event.Data.BuyerEmail); email != "" {
		if local, _, ok := strings.Cut(email, "@"); ok {
			if tradeNo := strings.TrimSpace(local); strings.HasPrefix(tradeNo, "WAFFO_PANCAKE-") {
				topUp := model.GetTopUpByTradeNo(tradeNo)
				if topUp != nil && topUp.PaymentMethod == model.PaymentMethodWaffoPancake {
					return tradeNo, nil
				}
	...
	if orderID := strings.TrimSpace(event.Data.OrderID); orderID != "" {
		return "", fmt.Errorf("waffo pancake order not found for webhook orderId=%s", orderID)
	}
