...
func transitionTaskToFailure(
	...
	won, refundRecord, err := task.UpdateFailureWithRefund(oldStatus)
	if err != nil {
		return false, err
	}
	if won {
...
func sweepTimedOutTasks(ctx context.Context) {
	...
	for _, task := range tasks {
		...
		if isLegacy {
			...
			won, err := transitionTaskToFailure(ctx, task, reason, now)
			if err != nil {
				logger.LogError(ctx, fmt.Sprintf("sweepTimedOutTasks CAS update error for task %s: %v", task.TaskID, err))
				continue
			}
...
func TaskPollingLoop(ctx context.Context) {
	billinglifecycle.RunPeriodic(ctx, 15*time.Second, false, func() {
		RunTaskPollingOnce(context.WithoutCancel(ctx))
	})
}

// RunTaskPollingOnce completes one polling round. Cancellation is checked by
// TaskPollingLoop between rounds so an already-started billing round is not
// abandoned halfway through.
func RunTaskPollingOnce(ctx context.Context) {
	...
	for platform, tasks := range platformTask {
		...
		if len(nullTasks) > 0 {
			...
			for _, task := range nullTasks {
				won, err := transitionTaskToFailure(ctx, task, reason, now)
				if err != nil {
					logger.LogError(ctx, fmt.Sprintf("Fix null task_id task error for db task %d: %v", task.ID, err))
				} else if won {
...
func updateSunoTasks(ctx context.Context, channelId int, taskIds []string, taskM map[string]*model.Task) error {
	...
	if err != nil {
		...
		for _, upstreamID := range taskIds {
			if t, ok := taskM[upstreamID]; ok {
				if _, transitionErr := transitionTaskToFailure(ctx, t, reason, now); transitionErr != nil {
					logger.LogError(ctx, fmt.Sprintf("UpdateSunoTask failure transition error for task %s: %v", t.TaskID, transitionErr))
				}
			}
		}
		return err
	...
	for _, responseItem := range responseItems.Data {
		...
		var won bool
		if task.Status == model.TaskStatusFailure && task.Quota > 0 && oldStatus != task.Status {
		...
		if err != nil {
			...
			logger.LogWarn(ctx, fmt.Sprintf("Suno task %s already transitioned, skip update and billing", task.TaskID))
		}
	}
	return nil
}
...
func updateVideoTasks(ctx context.Context, platform constant.TaskPlatform, channelId int, taskIds []string, taskM map[string]*model.Task) error {
	...
	if err != nil {
		...
		for _, upstreamID := range taskIds {
			if t, ok := taskM[upstreamID]; ok {
				if _, transitionErr := transitionTaskToFailure(ctx, t, reason, now); transitionErr != nil {
					logger.LogError(ctx, fmt.Sprintf("UpdateVideoTask failure transition error for task %s: %v", t.TaskID, transitionErr))
				}
			}
		}
