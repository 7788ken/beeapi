...
func ChargeViolationFeeIfNeeded(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, apiErr *types.NewAPIError) bool {
	...
	if err := model.UpdateUserAndChannelUsedQuota(relayInfo.UserId, relayInfo.ChannelId, feeQuota); err != nil {
		logger.LogError(ctx, "failed to update violation fee usage statistics: "+err.Error())
	}
