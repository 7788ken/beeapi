...
type ClaudeResponseInfo struct {
	...
	Usage                      *dto.Usage
	Done                       bool
	OpenAIToolCallIndexByBlock map[int]int
	NextOpenAIToolCallIndex    int
}
...
func StreamResponseClaude2OpenAI(claudeResponse *dto.ClaudeResponse) *dto.ChatCompletionsStreamResponse {
	return StreamResponseClaude2OpenAIWithInfo(claudeResponse, nil)
}

func StreamResponseClaude2OpenAIWithInfo(claudeResponse *dto.ClaudeResponse, claudeInfo *ClaudeResponseInfo) *dto.ChatCompletionsStreamResponse {
		...
		if claudeResponse.ContentBlock != nil {
			...
			if claudeResponse.ContentBlock.Type == "tool_use" {
				tools = append(tools, dto.ToolCallResponse{
					Index: common.GetPointer(openAIToolCallIndex(claudeInfo, fcIdx)),
...
func openAIToolCallIndex(claudeInfo *ClaudeResponseInfo, blockIndex int) int {
	if claudeInfo == nil {
		return blockIndex
	}
	if claudeInfo.OpenAIToolCallIndexByBlock == nil {
		claudeInfo.OpenAIToolCallIndexByBlock = make(map[int]int)
	}
	if index, ok := claudeInfo.OpenAIToolCallIndexByBlock[blockIndex]; ok {
		return index
	}
	index := claudeInfo.NextOpenAIToolCallIndex
	claudeInfo.OpenAIToolCallIndexByBlock[blockIndex] = index
	claudeInfo.NextOpenAIToolCallIndex++
	return index
}
