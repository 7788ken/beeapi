...
func TestBuildTieredTokenParams_OpenAICacheWriteTokens(t *testing.T) {
	usage := &dto.Usage{
		PromptTokens:     3619,
		CompletionTokens: 36,
		PromptTokensDetails: dto.InputTokenDetails{
	...
	if params.P != 0 {
		t.Fatalf("P = %f, want 0 after saturating cache read/write subtraction", params.P)
	}
	if params.CR != 2921 {
		t.Fatalf("CR = %f, want 2921", params.CR)
	}
	if params.CC != 3616 {
		t.Fatalf("CC = %f, want 3616", params.CC)
	}
	if params.Len != 3619 {
		t.Fatalf("Len = %f, want 3619", params.Len)
	}
}
