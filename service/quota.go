...
func calculateAudioQuota(info QuotaInfo) (int, *common.QuotaClamp) {
	if info.UsePrice {
		...
		// 保持原 int(quota.IntPart()) 的向零截断口径，仅增加溢出饱和保护
		return common.QuotaFromDecimalTruncateChecked(quota)
	}
...
func PostWssConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, modelName string,
	...
	if totalTokens == 0 {
		...
		if err := model.UpdateUserAndChannelUsedQuota(relayInfo.UserId, relayInfo.ChannelId, quota); err != nil {
...
func CalcOpenRouterCacheCreateTokens(usage dto.Usage, priceData types.PriceData) int {
	...
	// 分母 (promptCacheCreatePrice - quotaPrice) 接近 0 时结果可爆表或为负，
	// 饱和转换防止 int 回绕；派生 token 数为负无意义，置 0。
	tokens := common.QuotaFromFloat(math.Round((cost -
...
func PostAudioConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent string) {
	...
	_ = backgroundtask.Submit("audio-relay-sample-success", func(context.Context) {
...
func PostConsumeQuota(relayInfo *relaycommon.RelayInfo, quota int, preConsumedQuota int, sendEmail bool) (err error) {
	...
	if relayInfo.BillingSource == BillingSourceSubscription {
		...
		if !relayInfo.IsPlayground && relayInfo.TokenId > 0 {
			err = model.AdjustSubscriptionTokenQuota(
				relayInfo.UserId,
			...
			err = model.PostConsumeUserSubscriptionDelta(relayInfo.SubscriptionId, int64(quota))
		}
	} else {
		if !relayInfo.IsPlayground && relayInfo.TokenId > 0 {
			err = model.AdjustUserTokenQuota(
				relayInfo.UserId,
				relayInfo.TokenId,
				quota,
			)
...
// ConsumeMidjourneyQuota atomically marks the task as charged and applies its
// User/Subscription + Token mutation. A task whose charge fails keeps quota=0
// and therefore cannot refund unrelated usage during polling.
func ConsumeMidjourneyQuota(task *model.Midjourney, relayInfo *relaycommon.RelayInfo, quota int, sendEmail bool) error {
	if task == nil || relayInfo == nil {
		return errors.New("midjourney task or relay info is missing")
	}
	...
	if err := task.ConsumeQuota(relayInfo.UserId, tokenId, userSubscriptionId, quota); err != nil {
		return err
	}
...
func checkAndSendQuotaNotify(relayInfo *relaycommon.RelayInfo, quota int, preConsumedQuota int) {
	_ = backgroundtask.Submit("quota-notification", func(context.Context) {
		...
		if quotaTooLow {
			// 未充值账号不发余额提醒：这类用户低余额后同样每次请求都触发，会挤占真实客户的发信配额。
			if !IsPaidUser(relayInfo.UserId) {
				return
			}
...
func checkAndSendSubscriptionQuotaNotify(relayInfo *relaycommon.RelayInfo) {
	_ = backgroundtask.Submit("subscription-quota-notification", func(context.Context) {
		if relayInfo == nil {
			return
		}
