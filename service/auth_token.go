...
func ParseAccessToken(raw string) (AuthIdentity, error) {
	claims, err := parseAuthClaims(
		raw,
		accessTokenUse,
		authSigningKey(accessTokenUse),
	)
	if err != nil {
...
// ParseDashboardAccessToken distinguishes internal dashboard JWTs from opaque
// credentials. Once a token identifies itself as an internal dashboard token,
// signature, lifetime or purpose failures must not fall through to PAT
// authentication.
...
func IssueSecurityProof(
	...
	proofScopes := make([]string, 0, len(scopes))
	...
	for _, scope := range scopes {
		...
		proofScopes = append(proofScopes, scope)
	}
	now := time.Now()
...
func VerifySecurityProof(
	...
	claims, err := parseAuthClaims(
		raw,
		securityProofTokenUse,
		authSigningKey(securityProofTokenUse),
	)
	if err != nil {
		return "", err
	}
	if strings.TrimSpace(claims.Method) == "" || len(claims.Scopes) == 0 {
...
func parseAuthClaims(
	...
	parsed, err := jwt.ParseWithClaims(
		...
		jwt.WithIssuer(authTokenIssuer),
		jwt.WithAudience(authTokenAudience),
	...
	if !parsed.Valid || claims.TokenUse != expectedUse ||
		claims.ID == "" || claims.IssuedAt == nil || claims.NotBefore == nil {
		return nil, ErrAuthTokenInvalid
	}
	return claims, nil
}
