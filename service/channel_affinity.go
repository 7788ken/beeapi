...
// ShouldSkipRetryAfterChannelAffinityFailure 纯亲和逻辑：亲和命中渠道失败且规则要求 skip 时不重试。
// 重试范围（含 Key 级 relay_retry_policy / 渠道级 RetryStrategy）已统一到 service.EffectiveRetryScope，不在此处理。
