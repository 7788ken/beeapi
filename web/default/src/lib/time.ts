 ...
 * Special days values:
 *   0  -> today: 00:00:00 to 23:59:59 of fromDate
 *   -1 -> yesterday: yesterday 00:00:00 to 23:59:59
 *   N  -> last N * 24h ending at fromDate (rolling, not natural-day)
 */
export function getRollingDateRange(
  ...
  if (days === 0) {
    return { start: getStartOfDay(fromDate), end: getEndOfDay(fromDate) }
  }
  if (days === -1) {
    const y = new Date(fromDate)
