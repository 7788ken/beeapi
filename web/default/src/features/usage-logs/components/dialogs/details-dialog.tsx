...
export function DetailsDialog(props: DetailsDialogProps) {
  ...
  const reportInfo = [
    `Request ID: ${props.log.request_id || '-'}`,
    props.log.upstream_request_id
      ? `Upstream Request ID: ${props.log.upstream_request_id}`
      : '',
    `Time (UTC): ${props.log.created_at ? new Date(props.log.created_at * 1000).toISOString() : '-'}`,
    `Model: ${props.log.model_name || '-'}`,
    `Tokens: ${props.log.prompt_tokens} + ${props.log.completion_tokens}`,
    `Token: ${props.log.token_name || '-'}`,
    props.log.content ? `Detail: ${props.log.content}` : '',
  ]
    .filter(Boolean)
    .join('\n')
        ...
        <DialogHeader className='max-sm:gap-1'>
          <DialogTitle className='flex items-center gap-2 text-base'>
            ...
            <Button
              ...
              onClick={() => copyToClipboard(reportInfo)}
              title={t('Copy this request key info to report to the provider')}
            >
              {copiedText === reportInfo ? (
