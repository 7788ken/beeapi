...
export function useCommonLogsColumns(isAdmin: boolean): ColumnDef<UsageLog>[] {
    ...
    columns.push(
      {
        id: 'channel',
        accessorKey: 'channel',
        // accessorKey 让 row.channel(number) 暴露给 client filter；
        // 显式 includesString，避免 auto 走 inNumberRange（resolveFilterValue
        // 会把单值字符串解构成 [首字符, 次字符]，'10' → [1,0] 把所有行干掉）
  ...
  columns.push({
    ...
    cell: function TokenNameCell({ row }) {
      ...
      const groupLabel = sensitiveVisible ? group : '••••'
      const groupLinkable = groupVisible && sensitiveVisible && isAdmin
