...
export function ModelMutateDrawer({
  ...
  const [pricingManagedExternally, setPricingManagedExternally] =
    useState(false)
  ...
  useEffect(() => {
    if (open && isEditing && modelData?.data) {
      ...
      setPricingManagedExternally(pricing.managedExternally)
      form.reset({
        id: model.id,
    ...
    async (values: ExtendedModelFormValues): Promise<void> => {
      ...
      try {
        ...
        if (response.success) {
          ...
          if (modelSettings) {
            const currentPricingMaps = parseModelPricingMaps(modelSettings)
            const pricingMaps = mutateModelPricingMaps(currentPricingMaps, {
              isEditing,
              oldModelName,
              finalModelName,
              loadedPricingName,
            ...
            for (const key of MODEL_PRICING_OPTION_KEYS) {
              if (
                modelPricingMapEquals(
                  pricingMaps[key] as Record<string, number | string>,
                  currentPricingMaps[key] as Record<string, number | string>
                )
              ) {
                continue
              }
              const value = normalizeJsonString(
                JSON.stringify(pricingMaps[key])
              )
              if (value !== normalizeJsonString(modelSettings[key])) {
        ...
        <Form {...form}>
          ...
          >
            ...
            <div className='space-y-4'>
              ...
              {pricingManagedExternally && (
                <FormDescription>
                  {t(
                    'Tiered expression pricing is preserved here. Edit it in System Settings > Models.'
                  )}
                </FormDescription>
              )}
              ...
              <div className='space-y-4'>
                ...
                <RadioGroup
                  value={pricingMode}
                  disabled={pricingManagedExternally}
                  onValueChange={(value) =>
