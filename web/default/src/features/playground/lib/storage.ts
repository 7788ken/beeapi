...
export function loadMessages(): Message[] | null {
  try {
    ...
    if (saved) {
      ...
      // Guard against legacy / corrupted localStorage shapes (e.g. older
      // playground stored an object instead of an array). Without this guard
      // `messages.map(...)` in PlaygroundChat throws and the whole route
      // boundary falls back to the 500 error page.
