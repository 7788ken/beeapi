...
import { UserRpmSection } from '../user-rpm/user-rpm-section'
...
// ── Tab definitions ──
// Each section becomes a tab with a distinct chart-color icon so the form is
// scannable at a glance. Color classes are written as literals so Tailwind's
// content scanner can detect and generate them.
type ChannelTabDef = {
  value: string
  label: string
...
const CHANNEL_TABS: ChannelTabDef[] = [
  ...
  {
    value: 'extra',
    label: 'Channel Extra Settings',
    icon: Settings,
    iconColor: 'text-chart-4',
  },
]

// User-level RPM lives in its own tab, only available when editing an existing
// channel (rules attach to a saved channel id).
const RPM_TAB: ChannelTabDef = {
  value: 'rpm',
  label: 'User-level RPM limit',
  icon: Gauge,
  iconColor: 'text-chart-5',
}

// Map each validatable field to the tab that owns it, so validation errors can
// surface a marker on the right tab and jump the user there on submit.
// Basic Information and API Access share the 'basic' tab.
const FIELD_TAB_MAP: Record<string, string> = {
  name: 'basic',
  type: 'basic',
  status: 'basic',
  openai_organization: 'basic',
  remark: 'basic',
  tag: 'basic',
  base_url: 'basic',
  key: 'basic',
  models: 'models',
  group: 'models',
  model_mapping: 'models', // ModelMappingEditor 渲染在「模型与分组」标签内
  status_code_mapping: 'routing',
  param_override: 'routing',
  header_override: 'routing',
  priority: 'routing',
  weight: 'routing',
  test_model: 'routing',
  routing_mode: 'routing',
  capacity_limit: 'routing',
  capacity_window_sec: 'routing',
  capacity_full_strategy: 'routing',
  retry_strategy: 'routing',
  cache_domain: 'routing',
  ratio_panel_url: 'routing',
  ratio_upstream_group: 'routing',
  ratio_expected: 'routing',
}

function ChannelTabTrigger({
  tab,
  label,
  hasError,
}: {
...
export function ChannelMutateDrawer({
  ...
  // RPM tab only shows when editing (rules attach to a saved channel id).
  const visibleTabs = isEditing ? [...CHANNEL_TABS, RPM_TAB] : CHANNEL_TABS
  ...
  useEffect(() => {
    ...
    if (currentType === CHANNEL_TYPE_ADVANCED_CUSTOM) {
      ...
      if (!currentAdvancedCustom?.trim()) {
        form.setValue(
          'advanced_custom',
          stringifyAdvancedCustomConfig(createAdvancedCustomConfig())
        )
      }
    }
  ...
  const fetchChannelKey = useCallback(
    async (proof?: string) => {
      ...
      try {
        const res = await getChannelKey(channelId, proof)
        if (!res.success) {
  ...
  const handleFetchModels = useCallback(async () => {
    ...
    if (isEditing && currentRow && type === CHANNEL_TYPE_ADVANCED_CUSTOM) {
      ...
      try {
        ...
        if (!response.success || !response.data) {
          throw new Error(
            response.message || t('No models fetched from upstream')
          )
        }
  ...
  const onSubmit = useCallback(
    async (data: ChannelFormValues) => {
      ...
      if (!isEditing && !data.key?.trim()) {
        ...
        // key 在 basic 标签：手动 setError 不会触发 onInvalid，主动跳过去让用户看到错误
        setActiveTab('basic')
        return
      }
  ...
  // Which tabs currently hold a validation error (for the red marker dots).
  const formErrors = form.formState.errors
  const tabErrorMap = useMemo(() => {
    const map: Record<string, boolean> = {}
    for (const fieldName of Object.keys(formErrors)) {
      const tab = FIELD_TAB_MAP[fieldName]
      if (tab) map[tab] = true
    }
    return map
  }, [formErrors])

  // On failed submit, jump to the first tab that has an error so it isn't
  // hidden behind an inactive tab.
  const onInvalid = useCallback((errors: FieldErrors<ChannelFormValues>) => {
    const firstTab = CHANNEL_TABS.find((tabDef) =>
      Object.keys(errors).some((field) => FIELD_TAB_MAP[field] === tabDef.value)
    )
      ...
      <Sheet open={open} onOpenChange={handleOpenChange}>
        <SheetContent className='flex h-dvh w-full flex-col gap-0 overflow-hidden p-0 sm:max-w-3xl'>
          ...
          <Form {...form}>
            ...
            >
              <div className='bg-background/95 supports-[backdrop-filter]:bg-background/80 sticky top-0 z-10 border-b px-3 pt-3 backdrop-blur sm:px-4'>
                <TabsList className='flex h-auto w-full flex-wrap justify-start gap-1 rounded-none border-0 bg-transparent p-0'>
                  {visibleTabs.map((tab) => (
                    <ChannelTabTrigger
                      key={tab.value}
                      tab={tab}
                      label={t(tab.label)}
                      hasError={tabErrorMap[tab.value]}
                    />
                  ))}
                </TabsList>
              </div>

              <div className='min-h-0 flex-1 overflow-y-auto px-3 py-3 pb-4 sm:px-4'>
                <form
                  id='channel-form'
                ...
                >
                  ...
                  >
                    ...
                    <div className='bg-card space-y-4 rounded-xl border p-5'>
                      ...
                      {/* Backup base URLs — failover when the primary host is unreachable */}
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormDescription>
                                {t(
                                  'Configure explicit incoming path, upstream path, converter, model rules and authentication. Unsupported or unmatched routes fail fast.'
                                )}
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                      ...
                      {![3, 8, 22, 36, 45].includes(currentType) && (
                        <FormField
                          control={form.control}
                          name='backup_base_urls'
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>
                                {t(
                                  'Backup Base URLs (one per line, auto-failover when primary is unreachable)'
                                )}
                              </FormLabel>
                              <FormControl>
                                <Textarea
                                  rows={3}
                                  placeholder={
                                    'https://sin-relay.example.com\nhttps://hk-relay.example.com'
                                  }
                                  {...field}
                                />
                              </FormControl>
                              <FormDescription>
                                {t(
                                  'One full base URL per line. When the primary base URL host has no network response, requests automatically switch to a backup URL. Do not add /v1 or trailing slash.'
                                )}
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                      ...
                      <FormField
                        ...
                        render={() => (
                          <FormItem>
                            ...
                            {modelMappingGuardrail.exposedTargetModels.length >
                              0 && (
                  ...
                  >
                    ...
                    <div className='bg-card space-y-4 rounded-xl border p-5'>
                      ...
                      <div className='space-y-4'>
                        ...
                        {/* 路由模式（docs/2026-05-26-channel-routing-mode-switchable.md） */}
                        <FormField
                          control={form.control}
                          name='routing_mode'
                          render={({ field }) => {
                            // 4 项路由模式 = routing_mode + capacity_full_strategy 复合：
                            // 桶模式/严格限流底层都是 capacity(2)，靠 full_strategy 区分软(fallback)/硬(queue)。
                            const cfs = form.watch('capacity_full_strategy')
                            let choice = 'inherit'
                            if (field.value === 1) choice = 'probabilistic'
                            else if (field.value === 2)
                              choice =
                                cfs === 'queue' || cfs === 'reject'
                                  ? 'strict'
                                  : 'bucket'
                            const pickStrategy = (s: string | null) =>
                              form.setValue('capacity_full_strategy', s, {
                                shouldDirty: true,
                              })
                            return (
                              <FormItem>
                                ...
                                >
                                  ...
                                  <SelectContent>
                                    <SelectItem value='inherit'>
                                      {t('Inherit from global (default)')}
                                    </SelectItem>
                                    <SelectItem value='probabilistic'>
                                      {t('Weighted (probabilistic)')}
                                    </SelectItem>
                                    <SelectItem value='bucket'>
                                      {t('Bucket (soft · degrade when full)')}
                                    </SelectItem>
                                    <SelectItem value='strict'>
                                      {t(
                                        'Strict rate limit (hard · queue then 429)'
                                      )}
                                    </SelectItem>
                                  </SelectContent>
                                </Select>
                                <FormDescription>
                                  {t(
                                    'Bucket = keep serving when full (soft, degrade); Strict rate limit = queue then 429 when full (hard). Inherit follows the global setting.'
                                  )}
                                </FormDescription>
                                <FormMessage />
                              </FormItem>
                          ...
                          <div className='grid gap-4 sm:grid-cols-2'>
                            <FormField
                              ...
                              render={({ field }) => (
                                <FormItem>
                                  <FormLabel>
                                    {t('Capacity limit (requests/window)')}
                                  </FormLabel>
                                  <FormControl>
                                    <Input
                                      type='number'
                                      placeholder={t(
                                        'Leave empty to reuse weight'
                                      )}
                                      value={field.value ?? ''}
                                      onChange={(e) => {
                                  ...
                                  <FormDescription>
                                    {t(
                                      'Max requests per window for this channel. With window = 60s this is the per-minute cap (RPM). Leave empty to reuse the weight value.'
                                    )}
                                  </FormDescription>
                                  <FormMessage />
                                </FormItem>
                            ...
                            <FormField
                              control={form.control}
                              name='capacity_window_sec'
                              render={({ field }) => (
                                <FormItem>
                                  <FormLabel>
                                    {t('Capacity window (seconds)')}
                                  </FormLabel>
                                  <FormControl>
                                    <Input
                                      type='number'
                                      placeholder={t(
                                        'Leave empty to inherit global'
                                      )}
                                      value={field.value ?? ''}
                                      onChange={(e) => {
                                        const v = e.target.value
                                  ...
                                  <FormDescription>
                                    {t(
                                      'Per-channel sliding window. Set to 60 so capacity limit equals requests per minute (RPM). Leave empty to inherit global.'
                                    )}
                                  </FormDescription>
                                  <FormMessage />
                                </FormItem>
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>{t('Retry mode')}</FormLabel>
                              <Select
                              ...
                              >
                                ...
                                <SelectContent>
                                  ...
                                  <SelectItem value='1'>
                                    {t(
                                      'Cost guard (fail fast, no retry at all)'
                                    )}
                                  </SelectItem>
                                  <SelectItem value='2'>
                                    {t('Same cache domain only')}
                                  </SelectItem>
                                  <SelectItem value='3'>
                                    {t('Cross-channel (may lose cache)')}
                                  </SelectItem>
                                </SelectContent>
                              </Select>
                              <FormDescription>
                                {t(
                                  'How far a failed request may roam on retry. Cost guard / same cache domain protect prompt cache to avoid large re-billing; cross-channel maximizes success but may lose cache.'
                                )}
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                        ...
                        {/* cache_domain 是与重试模式无关的上游账号身份（Key 级同域策略也用），常驻显示，避免隐藏后残值不可见仍生效 */}
                        <FormField
                          control={form.control}
                          name='cache_domain'
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>{t('Cache domain')}</FormLabel>
                              <FormControl>
                                <Input
                                  maxLength={64}
                                  placeholder={t(
                                    'e.g. anthropic-account-A (channels sharing one upstream account)'
                                  )}
                                  {...field}
                                  value={field.value ?? ''}
                                />
                              ...
                              <FormDescription>
                                {t(
                                  'Upstream account identity: channels with the same cache domain share prompt cache. Used by both the channel-level "Same cache domain only" mode and the key-level same-domain policy. Empty = this channel only.'
                                )}
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                        ...
                        <FormField
                          control={form.control}
                          name='ratio_panel_url'
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>{t('Ratio panel URL')}</FormLabel>
                              <FormControl>
                                <Input
                                  maxLength={255}
                                  placeholder={t(
                                    'e.g. https://www.ai-wave.org (leave empty to use the channel URL)'
                                  )}
                                  {...field}
                                  value={field.value ?? ''}
                                />
                              ...
                              <FormDescription>
                                {t(
                                  'Used only for upstream group ratio monitoring. Fill this in when the upstream panel domain differs from the relay URL above; leave empty to reuse the channel URL.'
                                )}
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                        ...
                        <FormField
                          control={form.control}
                          name='ratio_upstream_group'
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>{t('Upstream group name')}</FormLabel>
                              <FormControl>
                                <Input
                                  maxLength={191}
                                  placeholder={t(
                                    'e.g. gemini / claude_max (leave empty to auto-detect)'
                                  )}
                                  {...field}
                                  value={field.value ?? ''}
                                />
                              ...
                              <FormDescription>
                                {t(
                                  'The group this channel key belongs to on the upstream. Upstreams do not expose it, so it is auto-detected by comparing model lists; fill it in when detection is ambiguous, otherwise the list can only show a ratio range.'
                                )}
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>{t('Expected purchase ratio')}</FormLabel>
                              <FormControl>
                                <Input
                                  type='number'
                                  step='0.001'
                                  min='0'
                                  placeholder={t('e.g. 0.11 (leave empty to skip the check)')}
                                  value={field.value ?? ''}
                                  onChange={(e) =>
                                    field.onChange(
                                      e.target.value === ''
                              ...
                              <FormDescription>
                                {t(
                                  'The ratio you actually negotiated with this upstream. Compared against the measured ratio; you get notified when the upstream charges more than this, which catches a silently revoked discount.'
                                )}
                              </FormDescription>
                              <FormMessage />
                            </FormItem>
                          ...
                          <div className='divide-border space-y-0 divide-y border-y'>
                            ...
                            {currentType === 14 && (
                              <>
                                ...
                                <FormField
                                  ...
                                  render={({ field }) => (
                                    <FormItem className='px-4 py-3'>
                                      <FormLabel className='text-sm'>
                                        {t('Auto-verify interval (minutes)')}
                                      </FormLabel>
                                      <FormControl>
                                        <Input
                                          type='number'
                                          min={0}
                                          placeholder={t(
                                            'Leave empty to follow global; 0 to disable'
                                          )}
                                          value={field.value ?? ''}
                                          onChange={(e) => {
                                            const v = e.target.value
                                      ...
                                      <FormDescription>
                                        {t(
                                          'Per-channel auto-verify interval. Empty = follow global, 0 = off.'
                                        )}
                                      </FormDescription>
                                      <FormMessage />
                                    </FormItem>
                                ...
                                <FormField
                                  control={form.control}
                                  name='verify_auto_disable_below'
                                  render={({ field }) => (
                                    <FormItem className='px-4 py-3'>
                                      <FormLabel className='text-sm'>
                                        {t('Auto-disable below verify score')}
                                      </FormLabel>
                                      <FormControl>
                                        <Input
                                          type='number'
                                          ...
                                          max={100}
                                          placeholder={t(
                                            'Leave empty to not auto-disable'
                                          )}
                                          value={field.value ?? ''}
                                          onChange={(e) => {
                                            const v = e.target.value
                                      ...
                                      <FormDescription>
                                        {t(
                                          'Auto-disable this channel when its verify score is below this value.'
                                        )}
                                      </FormDescription>
                                      <FormMessage />
                                    </FormItem>
                                ...
                                <FormField
                                  control={form.control}
                                  name='verify_auto_enable_above'
                                  render={({ field }) => (
                                    <FormItem className='px-4 py-3'>
                                      <FormLabel className='text-sm'>
                                        {t('Auto-enable at/above verify score')}
                                      </FormLabel>
                                      <FormControl>
                                        <Input
                                          type='number'
                                          ...
                                          max={100}
                                          placeholder={t(
                                            'Leave empty to not auto-enable'
                                          )}
                                          value={field.value ?? ''}
                                          onChange={(e) => {
                                            const v = e.target.value
                                      ...
                                      <FormDescription>
                                        {t(
                                          'Auto-enable a verify-disabled channel when score reaches this value (should be >= the disable threshold).'
                                        )}
                                      </FormDescription>
                                      <FormMessage />
                                    </FormItem>
                ...
                {/* 用户级 RPM 限流：仅在编辑已存在渠道时可用，独立标签页 */}
                {isEditing && (
                  <TabsContent
                    value='rpm'
                    className='mt-0 focus-visible:outline-none'
                  >
                    <UserRpmSection channelId={channelId} />
                  </TabsContent>
                )}
              </div>
            </Tabs>
