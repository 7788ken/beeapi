...
/** 当前分组倍率摘要（后端 channels.ratio_detail 的 JSON） */
export interface RatioSummary {
  n: number
  min: number
  max: number
  /** 已定位到我方 key 所属分组时的分组名；空=只能给全表区间 */
  g?: string
}

/** 解析倍率摘要；空/非法返回 null（表示尚未抓取到） */
export function parseRatioSummary(raw?: string | null): RatioSummary | null {
  if (!raw) return null
  try {
    ...
    return { n: parsed.n ?? 1, min: parsed.min, max: parsed.max, g: parsed.g }
  } catch {
    return null
  }
}

/** 单组显示确切值，多组显示区间；末尾多余的 0 去掉 */
export function formatRatioSummary(s: RatioSummary): string {
