...
/**
 * Calculate payment amount for Cryptomus payment
 */
export async function calculateCryptomusAmount(
  request: AmountRequest
): Promise<AmountResponse> {
...
/**
 * Request Cryptomus payment
 */
export async function requestCryptomusPayment(
  request: CryptomusPaymentRequest
): Promise<CryptomusPaymentResponse> {
  const res = await api.post('/api/user/cryptomus/pay', request, {
    skipBusinessError: true,
  } as Record<string, unknown>)
  return res.data
}
...
/**
 * Calculate payment amount for agou payment
 */
export async function calculateAgouAmount(
  request: AmountRequest
): Promise<AmountResponse> {
  const res = await api.post('/api/user/sfpay/amount', request, {
    skipBusinessError: true,
  } as Record<string, unknown>)
  return res.data
}
...
/**
 * Request agou payment（返回收银台 url，前端整页跳转）
 */
export async function requestAgouPayment(
  request: AgouPaymentRequest
): Promise<AgouPaymentResponse> {
  const res = await api.post('/api/user/sfpay/pay', request, {
    skipBusinessError: true,
  } as Record<string, unknown>)
  return res.data
}
