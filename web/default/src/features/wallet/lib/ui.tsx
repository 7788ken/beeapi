...
import { SiAlipay, SiWechat, SiStripe, SiTether, SiBitcoin, SiEthereum } from 'react-icons/si'
...
export function getPaymentIcon(
  ...
  switch (paymentType) {
    ...
    case 'tether':
    case 'usdt':
      return <SiTether className={className} style={{ color: '#26A17B' }} />
    case 'bitcoin':
    case 'btc':
...
/**
 * 渲染支付渠道/平台图标：icon 为图片 URL（/pay-card.png 或 http）或 base64 data:image
 * 时走 <img>，否则当作 react-icons 图标键（alipay/wechat/tether/bitcoin/ethereum）解析。
 */
export function getChannelIcon(
  ...
  return getPaymentIcon(icon, className, undefined, name)
}
