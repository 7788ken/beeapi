...
export const userSchema = z.object({
  ...
  // 实时 RPM（最近 60s 滑动窗口）；字段名 rpm_24h 保留向后兼容，数值由 controller.overlayRealtimeUserRPM 用 Redis 值覆盖
  rpm_24h: z.number().default(0),
  rpm_updated_at: z.number().default(0),
...
export interface GetUsersParams {
  p?: number
  page_size?: number
  // 接口级排序：order_by 必须在后端白名单内（id/created_at/last_login_at/used_quota/request_count/rpm_24h/quota）。
  order_by?: string
  order?: 'asc' | 'desc'
}
...
export interface UserFormData {
  ...
  rpm_limit?: number // Per-user RPM limit (0 = unlimited)
  pinned?: boolean // Pin user to quick access panel
  inviter_id?: number // Inviter user ID
  expires_at?: number // 账号有效期 unix sec；0 = 永不过期
}
