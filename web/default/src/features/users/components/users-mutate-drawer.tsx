              ...
              {isUpdate && (
                <div className='space-y-4'>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          {t(
                            'Per-user requests per minute limit. 0 means unlimited.'
                          )}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>
                          {t('User ID of the inviter. 0 means no inviter.')}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    name='expires_at'
                    render={({ field }) => {
                      // unix sec ↔ datetime-local string；空字符串视为 0（永不过期）
                      const tsToLocalInput = (ts: number | undefined): string => {
                        if (!ts || ts <= 0) return ''
                      ...
                      return (
                        <FormItem>
                          <FormLabel>{t('Account Expiry')}</FormLabel>
                          <FormControl>
                            <Input
                              type='datetime-local'
                              value={tsToLocalInput(field.value)}
                              onChange={(e) => {
                                ...
                                field.onChange(isNaN(ts) ? 0 : ts)
                              }}
                            />
                          </FormControl>
                          <FormDescription>
                            {t(
                              'Account expiry timestamp. Empty / 0 = never expires. After this time, user cannot log in.'
                            )}
                          </FormDescription>
                          <FormMessage />
                        </FormItem>
                    ...
                    render={({ field }) => (
                      <FormItem className='flex items-center justify-between rounded-lg border p-3'>
                        <div className='space-y-0.5'>
                          <FormLabel>{t('Pin to Quick Access')}</FormLabel>
                          <FormDescription>
                            {t('Show this user in the pinned users panel.')}
                          </FormDescription>
                        </div>
                        <FormControl>
                          <Switch
