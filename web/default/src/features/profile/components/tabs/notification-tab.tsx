...
export function NotificationTab({ profile, onUpdate }: NotificationTabProps) {
  ...
  const [settings, setSettings] = useState<UserSettings>({
    notify_type: 'email',
    quota_warning_threshold: DEFAULT_QUOTA_WARNING_THRESHOLD / quotaPerUnit,
    notification_email: '',
    webhook_url: '',
    webhook_secret: '',
    bark_url: '',
    ...
    price_change_notify_disabled: false,
  })
  ...
  useEffect(() => {
    if (profile?.setting) {
      ...
      setSettings({
        ...
        price_change_notify_disabled:
          parsed.price_change_notify_disabled || false,
      })
    }
  }, [profile])
  ...
  const handleSave = async () => {
    try {
      ...
      const thresholdUsd =
        ...
        DEFAULT_QUOTA_WARNING_THRESHOLD / quotaPerUnit
      const response = await updateUserSettings({
        ...settings,
        quota_warning_threshold: Math.round(thresholdUsd * quotaPerUnit),
      })
  ...
  return (
    <div className='space-y-4 sm:space-y-6'>
      ...
      <div className='space-y-3'>
        ...
        {/* Accept Unset Model Price (admin only) */}
        {isAdmin && (
          <div className='flex items-start justify-between gap-3 rounded-lg border p-3 sm:items-center sm:p-4'>
            <div className='space-y-0.5'>
        ...
        {/* Price Change Notifications (opt-out) */}
        <div className='flex items-start justify-between gap-3 rounded-lg border p-3 sm:items-center sm:p-4'>
          <div className='space-y-0.5'>
            <Label htmlFor='priceChangeNotify'>
              {t('Price Change Notifications')}
            </Label>
            <p className='text-muted-foreground text-xs sm:text-sm'>
              {t(
                'Receive a notification when published price changes affect your group'
              )}
            </p>
          </div>
          <Switch
            id='priceChangeNotify'
            className='shrink-0'
            checked={!settings.price_change_notify_disabled}
            onCheckedChange={(checked) =>
              updateField('price_change_notify_disabled', !checked)
            }
          />
        </div>
