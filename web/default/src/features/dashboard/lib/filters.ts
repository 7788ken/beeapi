...
// 旧版 preset days → 新版 days 的迁移表
// 1=过去 24h（≈ 今天），14/29=被淘汰的旧选项 → 默认今天
const LEGACY_TIME_RANGE_DAYS_MAP: Record<number, number> = {
  1: 0,
  14: 7,
  29: 30,
}
...
export function getSavedChartPreferences(): DashboardChartPreferences {
  ...
  try {
    ...
    let migratedDays: number
    if (isTimeRangePresetDays(parsed.defaultTimeRangeDays)) {
      migratedDays = parsed.defaultTimeRangeDays
    } else if (
      typeof parsed.defaultTimeRangeDays === 'number' &&
      parsed.defaultTimeRangeDays in LEGACY_TIME_RANGE_DAYS_MAP
    ) {
      migratedDays = LEGACY_TIME_RANGE_DAYS_MAP[parsed.defaultTimeRangeDays]
    } else {
      migratedDays = fallbackPreferences.defaultTimeRangeDays
    }

    const validated: DashboardChartPreferences = {
      ...
      defaultTimeRangeDays: migratedDays,
    ...
    // 一次性写回 localStorage，避免每次刷新都做一次静默修正
    if (parsed.defaultTimeRangeDays !== validated.defaultTimeRangeDays) {
      try {
