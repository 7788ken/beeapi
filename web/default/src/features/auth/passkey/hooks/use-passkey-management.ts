...
export function usePasskeyManagement(
  ...
  const register = useCallback(
    async (proof?: string) => {
      ...
      setRegistering(true)
      try {
        const beginResponse = await beginPasskeyRegistration(proof)
        ...
        if (!flowToken) {
          toast.error(i18next.t('Failed to start Passkey registration'))
          return false
        }
  ...
  const remove = useCallback(
    async (proof?: string) => {
      if (!proof) {
        toast.error(i18next.t('Security verification is required'))
        return false
      }
      setRemoving(true)
      try {
        const res = await deletePasskey(proof)
        if (!res.success) {
