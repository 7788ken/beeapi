...
type ApiCall = ((proof?: string) => Promise<unknown>) | null
...
export function useSecureVerification(
  ...
  const startVerification = useCallback(
    async (
      apiCall: (proof?: string) => Promise<unknown>,
  ...
  const executeVerification = useCallback(
    async (method?: VerificationMethod, code?: string) => {
      ...
      try {
        if (!state.scope) {
          throw new Error('Verification scope is required')
        }
        ...
        const result = await state.apiCall(proof)
