...
let pendingTwoFAFlowToken: string | null = null

export function setPendingTwoFAFlowToken(token: string | null) {
  pendingTwoFAFlowToken = token
}
...
export async function login2fa(payload: TwoFAPayload) {
  if (!pendingTwoFAFlowToken) throw new Error('Missing 2FA login flow')
  const res = await api.post<Login2FAResponse>('/api/user/login/2fa', {
    ...payload,
    flow_token: pendingTwoFAFlowToken,
  })
  pendingTwoFAFlowToken = null
  return res.data
}
