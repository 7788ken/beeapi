...
export function UserAuthForm({
    ...
    try {
      ...
      if (res.success) {
        if (res.data?.require_2fa) {
          setPendingTwoFAFlowToken(res.data.flow_token ?? null)
          redirectTo2FA()
          return
        }
  ...
  async function handlePasskeyLogin() {
    ...
    try {
      ...
      if (!flowToken) {
        throw new Error(t('Invalid Passkey login flow'))
      }
      const finish = await finishPasskeyLogin(assertion, flowToken)
      if (!finish.success) {
