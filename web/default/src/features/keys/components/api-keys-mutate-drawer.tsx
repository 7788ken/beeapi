...
import { type ApiKey, type ApiKeyGroupOption } from '../types'
import { GroupPickerField } from './group-picker-dialog'
...
export function ApiKeysMutateDrawer({
  ...
  const { triggerRefresh, presetGroup, setPresetGroup } = useApiKeys()
  ...
  // Hold presetGroup in a ref so it does NOT participate in the reset effect's
  // dependency array — otherwise the cycle "reset(group=foo) → setPresetGroup(null)
  // → effect re-fires with presetGroup=null → reset(defaults)" would clobber the
  // injected group right after we set it.
  const presetGroupRef = useRef<string | null>(presetGroup)
  useEffect(() => {
    presetGroupRef.current = presetGroup
  }, [presetGroup])

  // Load existing data when updating; for create, apply one-shot preset group
  // injected by the Group Square deep link (see PresetGroupEffect).
  useEffect(() => {
    if (open && isUpdate && currentRow) {
      ...
      const preset = presetGroupRef.current
      if (preset) {
        form.reset({ ...defaults, group: preset })
        setPresetGroup(null)
      } else {
        form.reset(defaults)
      }
  ...
  // Surface zod validation failures so users see why "Save changes" appears
  // unresponsive (otherwise handleSubmit silently swallows invalid form state).
  const onInvalid = (errors: FieldErrors<ApiKeyFormValues>) => {
    const firstError = Object.values(errors).find(
      (err) => err && typeof err.message === 'string' && err.message
    )
    toast.error(
  ...
  return (
    ...
    >
      <SheetContent
        side={side}
        className='bg-background flex w-[100vw]! left-0! max-w-none gap-0 overflow-hidden p-0 lg:left-auto! lg:w-[620px]! lg:max-w-[620px]!'
      >
        ...
        <Form {...form}>
          ...
          >
            ...
            >
              ...
              {/* Key 级重试策略（与渠道级取最保守，docs/2026-06-07-claude-retry-cache-loss.md） */}
              <FormField
                control={form.control}
                name='relay_retry_policy'
                render={({ field }) => (
                  <FormItem>
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value='disabled'>
                          {t('Cost guard (fail fast, no retry at all)')}
                        </SelectItem>
                        <SelectItem value='cache_domain_only'>
                          {t('Same cache domain only')}
                        </SelectItem>
                        <SelectItem value='allow_cross_channel'>
