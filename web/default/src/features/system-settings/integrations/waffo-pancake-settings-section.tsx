...
export function WaffoPancakeSettingsSection(props: Props) {
  ...
  const [payChannels, setPayChannels] = useState<PayChannelItem[]>(() =>
    parsePayChannels(props.defaultValues.WaffoPancakePayChannels)
  )
  const [logo, setLogo] = useState(props.defaultValues.WaffoPancakeLogo || '')
  ...
  useEffect(() => {
    ...
    setLogo(props.defaultValues.WaffoPancakeLogo || '')
  ...
  const handleSave = async () => {
    ...
    try {
      const options: { key: string; value: string }[] = [
        ...
        {
          key: 'WaffoPancakeAllowedGroups',
          value: values.WaffoPancakeAllowedGroups || '',
        },
        {
          key: 'WaffoPancakePayChannels',
          value: JSON.stringify(payChannels),
        },
        { key: 'WaffoPancakeLogo', value: logo },
      ]
    ...
    >
      ...
      <div className='grid gap-1.5'>
        ...
        <Input
          ...
          {...form.register('WaffoPancakeAllowedGroups')}
        />
        <p className='text-muted-foreground text-xs'>
          {t(
      ...
      <PayChannelsEditor
        ...
        paramFields={[]}
        iconHint={t(
          'Display only — Waffo hosts the checkout. Icon: react-icons key (e.g. visa) or image URL like /pay-card.png'
        )}
      />
