...
import { TokenHealthSection } from './token-health-section'
...
const REQUEST_LIMITS_SECTIONS = [
  ...
  {
    id: 'token-health',
    titleKey: 'Token Error-Rate Circuit Breaker',
    descriptionKey:
      'Temporarily throttle an API key when its client-side error rate is too high',
    build: (settings: RequestLimitsSettings) => (
      <TokenHealthSection
        defaultValues={{
          'token_health_setting.enabled':
            settings['token_health_setting.enabled'],
          'token_health_setting.window_minutes':
            settings['token_health_setting.window_minutes'],
          'token_health_setting.min_requests':
            settings['token_health_setting.min_requests'],
          'token_health_setting.error_rate_threshold':
            settings['token_health_setting.error_rate_threshold'],
          'token_health_setting.cooldown_minutes':
            settings['token_health_setting.cooldown_minutes'],
          'token_health_setting.excluded_status_codes':
            settings['token_health_setting.excluded_status_codes'],
        }}
      />
    ),
  },
