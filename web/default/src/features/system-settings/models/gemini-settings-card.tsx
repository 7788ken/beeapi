...
const schema = z.object({
  gemini: z.object({
    ...
    supported_mime_types: z.string().superRefine((value, ctx) => {
      const result = validateJsonString(value, {
        predicate: (parsed) =>
          Array.isArray(parsed) &&
          parsed.every((item) => typeof item === 'string'),
        predicateMessage: 'Expected a JSON array of MIME types',
      })
      if (!result.valid) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
...
export function GeminiSettingsCard({ defaultValues }: GeminiSettingsCardProps) {
  ...
  const normalizedDefaultsRef = useRef<FlatGeminiSettings>({
    ...
    'gemini.supported_mime_types': normalizeJsonString(
      defaultValues.gemini.supported_mime_types
    ),
  ...
  const buildFormDefaults = (
    ...
    gemini: {
      ...
      supported_mime_types: formatJsonForTextarea(
        values.gemini.supported_mime_types
      ),
  ...
  const mimePlaceholder = useMemo(
    () => JSON.stringify(['audio/flac', 'audio/aac', 'image/png'], null, 2),
    []
  )
    ...
    >
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-6'>
          ...
          <FormField
            control={form.control}
            name='gemini.supported_mime_types'
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t('Supported Media MIME Types')}</FormLabel>
                <FormControl>
                  <Textarea rows={6} placeholder={mimePlaceholder} {...field} />
                </FormControl>
                <FormDescription>
                  {t(
                    'Accepts a JSON array of media MIME types allowed to pass through to Gemini. Save [] to disable local validation.'
                  )}
                </FormDescription>
                <FormMessage />
              </FormItem>
