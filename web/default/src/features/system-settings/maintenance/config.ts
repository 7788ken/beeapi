...
export type HeaderNavModulesConfig = {
  ...
  rankings: HeaderNavPricingConfig
  docs: boolean
  about: boolean
    ...
    Object.entries(parsed).forEach(([key, raw]) => {
      if (key === 'pricing' || key === 'rankings') {
        if (raw && typeof raw === 'object') {
          const rawAccess = raw as Record<string, unknown>
          const baseAccess = base[key] as HeaderNavPricingConfig
          result[key] = {
            enabled: toBoolean(rawAccess.enabled, baseAccess?.enabled ?? true),
            requireAuth: toBoolean(
              rawAccess.requireAuth,
              baseAccess?.requireAuth ?? false
            ),
          }
        }
        return
