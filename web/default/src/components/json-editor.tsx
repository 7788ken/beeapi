...
export function JsonEditor({
  ...
  // 初始 rows 必须从 prop value 同步解析，否则 mount 时 visual 模式会渲染空，
  // 而下方 useEffect 的 value !== jsonValue 守卫会跳过首次 parse。
  const buildRowsFromJson = (json: string): EditorRow[] => {
    try {
  ...
  const [rows, setRows] = useState<EditorRow[]>(() => buildRowsFromJson(value))
