...
function RotatingText({ texts }: { texts: string[] }) {
  ...
  return (
    <span className='relative block h-4 overflow-hidden'>
      ...
      >
        {texts.map((text) => (
          <span key={text} className='block h-4 leading-4 truncate'>
            {text}
          </span>
        ))}
      </span>
    ...
    <>
      ...
      <div className='grid flex-1 text-start text-sm leading-tight group-data-[collapsible=icon]:hidden'>
        ...
        <span className='truncate text-xs'>
          <RotatingText texts={[
            t('I will hold the line for you.'),
            t('Let me handle the complexity.'),
            t('Your code is in good hands.'),
            t('Think big, I will do the rest.'),
            t('Bugs fear me, not you.'),
            t('Ship it, I got your back.'),
          ]} />
        </span>
      </div>
      {canSwitchWorkspace && (
