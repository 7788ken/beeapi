...
function getStoredUser() {
  ...
  try {
    ...
    const hadStoredToken =
      'token' in user || 'access_token' in user || 'refresh_token' in user;
    delete user.token;
...
export const createCardProPagination = ({
  ...
  const totalText = `${t('显示第')} ${start} ${t('条 - 第')} ${end} ${t('条，共')} ${formatLogTotal(total, totalIsCapped)} ${t('条')}`;

  return (
    <>
      {/* 桌面端左侧总数信息 */}
      {(!isMobile || totalIsCapped) && (
        <span
