...
const UserSubscriptionsModal = ({ visible, onCancel, user, t, onSuccess }) => {
  ...
  const [editExpirySub, setEditExpirySub] = useState(null);
  const [editExpiryValue, setEditExpiryValue] = useState(null);
  ...
  const openEditExpiry = (sub) => {
    setEditExpirySub(sub);
    const initial = sub?.end_time ? new Date(sub.end_time * 1000) : new Date();
    setEditExpiryValue(initial);
    setEditExpiryVisible(true);
  };

  const closeEditExpiry = () => {
    if (editExpirySubmitting) return;
    setEditExpiryVisible(false);
    setEditExpirySub(null);
    setEditExpiryValue(null);
  };

  const submitEditExpiry = async () => {
    if (!editExpirySub?.id) return;
    if (!editExpiryValue) {
      showError(t('请选择新的到期时间'));
      return;
    }
    const ts = Math.floor(new Date(editExpiryValue).getTime() / 1000);
    if (!ts || ts <= 0) {
      showError(t('无效的到期时间'));
      return;
    }
    ...
    try {
      const res = await API.patch(
        `/api/subscription/admin/user_subscriptions/${editExpirySub.id}/expiry`,
        { end_time: ts },
      );
      if (res.data?.success) {
        const msg = res.data?.data?.message;
        showSuccess(msg ? msg : t('已更新'));
        setEditExpiryVisible(false);
        setEditExpirySub(null);
        setEditExpiryValue(null);
        await loadUserSubscriptions();
  ...
  return (
    ...
    >
      ...
      <Modal
        ...
        onCancel={closeEditExpiry}
        onOk={submitEditExpiry}
        confirmLoading={editExpirySubmitting}
        maskClosable={!editExpirySubmitting}
        closeOnEsc={!editExpirySubmitting}
        centered
        okText={t('保存')}
        cancelText={t('取消')}
      >
        <div className='space-y-3'>
          <div className='text-xs text-gray-500'>
            {t('当前到期时间')}: {formatTs(editExpirySub?.end_time)}
          </div>
          <DatePicker
            type='dateTime'
            value={editExpiryValue}
            onChange={(v) => setEditExpiryValue(v)}
            style={{ width: '100%' }}
            placeholder={t('选择新的到期时间')}
          />
          <div className='text-xs text-gray-400'>
            {t(
              '若新到期时间晚于当前时间，订阅会被激活；早于当前时间，订阅会被标记为过期并回退用户分组。',
            )}
          </div>
        </div>
      </Modal>
