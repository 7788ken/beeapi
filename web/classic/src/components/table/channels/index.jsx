...
const ChannelsPage = () => {
  ...
  const [healthChannel, setHealthChannel] = useState(null);

  // URL ?keyword=xxx → 自动填入搜索框并搜索（用于从日志双击跳转过来定位渠道）
  useEffect(() => {
    const kw = searchParams.get('keyword');
    if (!kw) return;
    if (!channelsData.formApi) return;
    if (appliedKeywordRef.current === kw) return;
    appliedKeywordRef.current = kw;
    channelsData.formApi.setValue('searchKeyword', kw);
    channelsData.searchChannels(channelsData.enableTagMode);
    // 应用一次后清掉 URL 参数
    const next = new URLSearchParams(searchParams);
    next.delete('keyword');
    setSearchParams(next, { replace: true });
  }, [searchParams, channelsData.formApi]);
  ...
  return (
    <>
      ...
      />
      <ChannelHealthModal
        visible={Boolean(healthChannel)}
        channel={healthChannel}
        onCancel={() => setHealthChannel(null)}
        onRecovered={channelsData.refresh}
      />
      ...
      <CardPro
        ...
        t={channelsData.t}
      >
        <ChannelsTable {...channelsData} setHealthChannel={setHealthChannel} />
      </CardPro>
    </>
  );
};
