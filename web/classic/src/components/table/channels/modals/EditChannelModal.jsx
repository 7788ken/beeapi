...
function type2secretPrompt(type) {
  ...
  switch (type) {
    ...
    case 23:
      return 'TokenHub 请输入单段 API Key；原生 TC3 按照 AppId|SecretId|SecretKey 输入';
    case 33:
...
const EditChannelModal = (props) => {
      ...
      try {
        const res = await API.post(
          '/api/channel/fetch_models',
          {
            ...
            type: inputs.type,
            advanced_custom: inputs.advanced_custom,
  ...
  const submit = async () => {
    ...
    if (localInputs.type === 59) {
      let advancedCustom;
      try {
        advancedCustom = JSON.parse(localInputs.advanced_custom || '');
      } catch (error) {
        showError(t('Advanced Custom 路由必须是合法的 JSON'));
        return;
      }
      if (
        !advancedCustom ||
        !Array.isArray(advancedCustom.advanced_routes) ||
        advancedCustom.advanced_routes.length === 0
      ) {
        showError(t('Advanced Custom 至少需要一条路由'));
        return;
      }
      settings.advanced_custom = advancedCustom;
                    ...
                    <Card className='!rounded-2xl shadow-sm border-0'>
                      ...
                      <Form.Select
                        ...
                        onChange={(value) => {
                          ...
                          if (value === 59 && !inputs.advanced_custom) {
                            handleInputChange(
                              'advanced_custom',
                              JSON.stringify(
                                {
                      ...
                      {inputs.type === 59 && (
                        <Form.TextArea
                          ...
                          extraText={t(
                            '显式配置入口路径、上游路径、转换器、模型规则和鉴权；未匹配路由会直接失败。',
                          )}
                        />
                      )}
