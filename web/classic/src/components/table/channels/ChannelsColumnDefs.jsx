...
export const getChannelsColumns = ({
  ...
  return [
    ...
    },
    {
      ...
      title: t('健康状态'),
      dataIndex: 'degrade_level',
      render: (_, record) => {
        if (record.children !== undefined) return null;
        const level = record.degrade_level ?? 0;
        const locked = record.permanent_disabled === 1;
        const disabled = record.status === 2 || record.status === 3;
        const label = locked ? t('锁定') : disabled ? t('禁用') : `L${level}`;
        const color =
          locked || disabled || level >= 8
            ? 'red'
            : level >= 4
              ? 'orange'
              : level > 0
        ...
        return (
          ...
          >
            <Tag
              ...
              shape='circle'
              className='cursor-pointer'
              onClick={() => setHealthChannel?.(record)}
            >
              {label}
            </Tag>
          </Tooltip>
    ...
    {
      key: COLUMN_KEYS.VERIFY_SCORE,
      title: t('评分'),
      dataIndex: 'verify_score',
      render: (score, record) => {
        if (record.type !== 14) {
        ...
        const grade = record.verify_grade || '';
        let color = 'grey';
        if (grade === 'A+' || grade === 'A') color = 'green';
        else if (grade === 'B') color = 'blue';
        else if (grade === 'C') color = 'orange';
        else if (grade === 'D' || grade === 'F') color = 'red';
        return (
          <Tooltip
            content={
              record.verify_tested_at
                ? t('上次测评') +
                  ' ' +
                  new Date(record.verify_tested_at * 1000).toLocaleString()
                : t('外部测评（外部测评网关）')
            }
          >
    ...
    {
      key: COLUMN_KEYS.RATIO_CHANGE,
      title: t('倍率'),
      dataIndex: 'ratio_change',
      // 上游分组倍率变化角标（docs/2026-08-05-upstream-group-ratio-monitor.md）
      // 只显示最近一批次且限 7 天窗口，超窗不显示，避免红点永久挂着。
      render: (text, record) => {
        if (record.children !== undefined) {
          return null;
        }
        const status = record.ratio_fetch_status || '';
        const msg = record.ratio_fetch_msg || '';
        const fetchedAt = record.ratio_fetched_at ?? 0;
        const changedAt = record.ratio_changed_at ?? 0;
        const up = record.ratio_up_count ?? 0;
        const down = record.ratio_down_count ?? 0;

        if (status === 'unsupported') {
          return (
            <Tooltip content={t('该上游不支持倍率查询') + (msg ? '：' + msg : '')}>
        ...
        if (status === 'error') {
          return (
            <Tooltip content={t('抓取上游倍率失败') + (msg ? '：' + msg : '')}>
              <Tag color='orange' type='ghost' shape='circle'>
                {t('异常')}
              </Tag>
            </Tooltip>
          );
        ...
        if (fetchedAt <= 0) {
          return (
            <Tooltip content={t('尚未抓取过该渠道的上游分组倍率')}>
              <span style={{ color: '#999', fontSize: 12 }}>{t('未抓取')}</span>
            </Tooltip>
          );
        }

        const windowStart = Math.floor(Date.now() / 1000) - 7 * 86400;
        const inWindow = changedAt > 0 && changedAt >= windowStart;

        // 主体是"此刻的上游倍率"，角标只是次要提示
        let summary = null;
        try {
          const parsed = JSON.parse(record.ratio_detail || '');
        ...
        const eff = record.ratio_effective;
        const hasEff = Number.isFinite(eff) && eff > 0;
        const eff2 = hasEff ? Number(eff.toFixed(2)) : null;
        if (!summary) {
          if (hasEff) {
            return (
              <Tooltip content={t('实测实付倍率（由上游账单反推）')}>
                <span style={{ fontSize: 12 }}>{eff2}</span>
              </Tooltip>
            );
          }
          return (
            <Tooltip content={t('尚未抓取到该渠道的上游分组倍率')}>
              <span style={{ color: '#999', fontSize: 12 }}>{t('未抓取')}</span>
            </Tooltip>
          );
        }
        const fmt = (v) => String(Number(v.toFixed(4)));
        // 分组固定倍率优先；未定位到分组时用实付反推兜底（0.01 精度）
        const ratioText = summary.g
          ? fmt(summary.min)
          : hasEff
            ? String(eff2)
          ...
          return (
            <Tooltip
              content={
                ...
                t('上次抓取') +
                ' ' +
                timestamp2string(fetchedAt)
              }
            >
              <span
                style={{ fontSize: 12, color: summary.g ? undefined : '#999' }}
              >
                {ratioText}
              </span>
            </Tooltip>
        ...
        return (
          <Tooltip
            content={
              t('上游分组倍率已变化') +
              '，' +
              timestamp2string(changedAt)
            }
          >
            <Space spacing={2}>
              <span style={{ fontSize: 12, fontWeight: 500 }}>{ratioText}</span>
              {up > 0 && (
                <Tag color='red' shape='circle'>
                  {'\u2191' + up}
                </Tag>
