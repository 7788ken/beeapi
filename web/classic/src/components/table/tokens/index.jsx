...
function TokensPage() {
  ...
  // 响应 URL ?group=xxx：自动打开新建令牌弹窗并预选分组（来自分组广场跳转）
  const [searchParams, setSearchParams] = useSearchParams();
  useEffect(() => {
    const presetGroup = searchParams.get('group');
    if (!presetGroup) return;
    if (typeof tokensData.setEditingToken !== 'function') return;
    if (typeof tokensData.setShowEdit !== 'function') return;
    tokensData.setEditingToken({ id: undefined, group: presetGroup });
