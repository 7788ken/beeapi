  ...
  const popoverContent = (
    <div style={{ width: 260 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        ...
        <Text type='tertiary'>{t('绑定渠道组')}</Text>
        <Text>{plan?.bound_group ? plan.bound_group : t('不限制')}</Text>
...
const renderBoundGroup = (text, record, t) => {
  const group = record?.plan?.bound_group || '';
  return (
    <Text type={group ? 'secondary' : 'tertiary'}>
      {group ? group : t('不限制')}
    </Text>
...
export const getSubscriptionsColumns = ({
  ...
  return [
    ...
    },
    {
      title: t('绑定渠道组'),
      width: 120,
      render: (text, record) => renderBoundGroup(text, record, t),
    },
    {
      title: t('操作'),
      dataIndex: 'operate',
