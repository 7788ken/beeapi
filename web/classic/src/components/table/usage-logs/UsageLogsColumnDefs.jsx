...
export const getLogsColumns = ({
  ...
  const goToChannel = (record) => {
    if (!navigate || !record) return;
    // 优先用渠道名称搜索；缺失时 fallback 到 channel id
    const kw =
      (record.channel_name && String(record.channel_name).trim()) ||
  ...
  const goToUser = (record) => {
    if (!navigate || !record) return;
    // 优先用 username 搜索；缺失时 fallback 到 user_id
    const kw =
      (record.username && String(record.username).trim()) ||
      (record.user_id !== undefined && record.user_id !== null
        ? String(record.user_id)
        : '');
    if (!kw) return;
    navigate(`/console/user?keyword=${encodeURIComponent(kw)}`);
  };
  return [
    {
          ...
          <Space>
            <span style={{ position: 'relative', display: 'inline-block' }}>
              <Tooltip
                content={
                  navigate
                    ? `${record.channel_name || t('未知渠道')}（${t('双击跳转渠道管理')}）`
      ...
      render: (text, record, index) => {
        return isAdminUser ? (
          <Tooltip
            content={
              navigate
                ? t('双击跳转用户管理；单击头像查看用户信息')
                : ''
            }
            position='top'
            trigger={navigate ? 'mouseEnter' : 'custom'}
          >
            <div
              onDoubleClick={(e) => {
                e.stopPropagation();
    ...
    },
    {
      ...
      render: (text, record, index) => {
        ...
        // 单击该 cell 复制其纯文本到剪贴板，不向上冒泡触发行展开
