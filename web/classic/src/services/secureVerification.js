...
export class SecureVerificationService {
  ...
  static async verify2FA(code, scope) {
    ...
    const proof = verifyResponse.data?.data?.proof;
    if (!proof) {
      throw new Error('验证凭证签发失败');
    }
    return proof;
  }
  ...
  static async verifyPasskey(scope) {
    try {
      ...
      if (!flowToken) {
        throw new Error('Passkey 验证流程无效');
      }
      ...
      const proof = finishResponse.data?.data?.proof;
      if (!proof) {
        throw new Error('Passkey 验证凭证签发失败');
      }
      return proof;
    } catch (error) {
...
export const createApiCalls = {
   ...
   */
  viewChannelKey: (channelId) => async (proof) => {
