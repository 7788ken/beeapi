...
var defaultModelRatio = map[string]float64{
	...
	// 简写别名（无日期后缀，部分管理员通配符渠道会接收这些写法）
	"claude-3-5-sonnet":                         1.5, // 默认对齐 -20241022
	"claude-sonnet-4":                           1.5, // 默认对齐 sonnet-4 系列
	"claude-opus-4":                             7.5, // 默认对齐 opus-4 系列最贵版（保守，避免简写被滥用）
...
func GetModelPrice(name string, printErr bool) (float64, bool) {
	name = formatBillingModelName(name)
...
func formatBillingModelName(name string) string {
	return model_setting.ResolveBillingModelName(FormatMatchingModelName(name))
}
