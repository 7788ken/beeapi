...
> 📖 **[完整功能与设计说明 → docs/FEATURES.md](docs/FEATURES.md)** —— 41 项自研功能与修复，每项含介绍 / 原理 / 实现方案。
...
- **zstd 请求体解压** —— 解码器强制同步模式，避免鉴权前中止的请求泄漏后台 goroutine
- **上游参数透传保真** —— AWS 显式零值不被 `omitempty` 吞、Qwen `thinking_budget` 按模型双闸透传、Ollama 非流式补齐 `tool_calls`、OpenAI→Claude 不再注入空 `tools`
- `STREAMING_TIMEOUT=0` 等非正值配置不再让每个流式请求 panic
...
- **零产出免单** —— RelayFormat 白名单 fail-closed，`client_gone` 加时长门槛防刷缓存；上游因客户内容触发风控而显式拒绝的不免单
...
- **管理员细粒度权限** —— 六个权限位，默认值等于加权限位之前的行为；唯独改渠道默认关（能改 `base_url` 就能骗走上游 key）
