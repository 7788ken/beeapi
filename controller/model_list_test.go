...
func TestListModelsGPT56AliasInheritsSolBilling(t *testing.T) {
	withSelfUseModeDisabled(t)
	...
	pricing, ok := pricingByModelName(model.GetPricing())["gpt-5.6"]
	require.True(t, ok)
	require.Equal(t, "tiered_expr", pricing.BillingMode)
	require.Equal(t, solExpr, pricing.BillingExpr)
}
