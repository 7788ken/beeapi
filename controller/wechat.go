	...
	if model.IsWeChatIdAlreadyTaken(wechatId) {
		...
		if common.RegisterEnabled {
			...
			if err := model.DB.Transaction(func(tx *gorm.DB) error {
				...
				return service.ClaimExternalIdentity(tx, "wechat", wechatId, user.Id)
			}); err != nil {
				c.JSON(http.StatusOK, gin.H{
					"success": false,
					"message": err.Error(),
...
func WeChatBind(c *gin.Context) {
	...
	if model.IsWeChatIdAlreadyTaken(wechatId) {
		...
		if err := existing.FillUserByWeChatId(); err != nil || existing.Id != user.Id {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
				"message": "该微信账号已被绑定",
			})
	...
	err = model.DB.Transaction(func(tx *gorm.DB) error {
		if err := service.ClaimExternalIdentity(tx, "wechat", wechatId, user.Id); err != nil {
			return err
		}
		return tx.Model(&model.User{}).Where("id = ?", user.Id).
			Update("wechat_id", wechatId).Error
	})
	if err != nil {
		common.ApiError(c, err)
		return
