...
func GetTopUpInfo(c *gin.Context) {
	// 当前用户分组 → 四个动态网关（Waffo/WaffoPancake/Cryptomus/Agou）各自的分组白名单判定
	// （空白名单=全部开放）。下面不据此隐藏渠道，只用于收集 groupBlockedMethods 给前端置灰。
	userGroup, _ := model.GetUserGroup(c.GetInt("id"), true)
	...
	enableAgou := isAgouTopUpEnabled()

	// 分组不可用的支付方式：不再隐藏渠道，改为收集类型给前端置灰（依旧显示全部渠道但不可用）。
	// 静态 PayMethods 按各自 allowed_groups；四个动态网关按各自分组白名单。
	groupBlockedMethods := []string{}
	for _, m := range payMethods {
		if !operation_setting.IsPayMethodAllowedForGroup(m, userGroup) {
			groupBlockedMethods = append(groupBlockedMethods, m["type"])
		}
	}
	if enableWaffo && !waffoGroupAllowed {
		groupBlockedMethods = append(groupBlockedMethods, model.PaymentMethodWaffo)
	}
	if enableWaffoPancake && !waffoPancakeGroupAllowed {
		groupBlockedMethods = append(groupBlockedMethods, model.PaymentMethodWaffoPancake)
	}
	...
	if enableCryptomus {
		providers = append(providers, buildTopUpProvider("cryptomus", "Cryptomus", setting.CryptomusLogo,
			setting.CryptomusMinTopUp, 0, !cryptomusGroupAllowed, setting.GetCryptomusPayChannels()))
	}
	if enableAgou {
		providers = append(providers, buildTopUpProvider("sfpay", "Sfpay", setting.AgouLogo,
			setting.AgouMinTopUp, setting.AgouMaxTopUp, !agouGroupAllowed, setting.GetAgouPayChannels()))
	}
	...
	data := gin.H{
		...
		"sfpay_pay_methods": func() interface{} {
			if enableAgou {
				return setting.GetAgouPayMethods()
			}
			return nil
		}(),
		"sfpay_min_topup":         setting.AgouMinTopUp,
		"sfpay_max_topup":         setting.AgouMaxTopUp,
		"creem_products":          setting.CreemProducts,
		"pay_methods":             payMethods,
		"providers":               providers,
		"group_blocked_methods":   groupBlockedMethods,
...
// buildTopUpProvider 组装单个支付平台给前端：过滤出启用渠道，剥离内部网关参数（Params）。
// logo 为平台级可配置图标（图片 URL 或 base64 dataURL），前端卡片优先展示。
func buildTopUpProvider(id, name, logo string, minTopUp, maxTopUp int, blockedByGroup bool, channels []constant.PayChannel) gin.H {
	visible := make([]gin.H, 0, len(channels))
	for _, ch := range channels {
		if !ch.Enabled {
			continue
		}
		visible = append(visible, gin.H{
			"key":  ch.Key,
			"name": ch.Name,
			"icon": ch.Icon,
		})
	...
	return gin.H{
		...
		"min_topup":        minTopUp,
		"max_topup":        maxTopUp,
		"blocked_by_group": blockedByGroup,
		"channels":         visible,
	}
}
...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		case errors.Is(err, model.ErrTopUpStatusInvalid):
			// 订单已被其他回调或节点结算，属于正常重复通知。此处不打印内存态 status：
			// 抢占失败时内存里仍是读取时的 pending，打出来会让人误判成"未结算却被跳过"
			// 而去手工补额度，那是历史上重复付钱的入口。真实状态以库为准。
			logger.LogInfo(c.Request.Context(), fmt.Sprintf("易支付 回调订单已结算或非待支付，跳过入账 trade_no=%s callback_type=%s client_ip=%s", verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP()))
			return
		case err != nil:
			logger.LogError(c.Request.Context(), fmt.Sprintf("易支付 结算充值订单失败 trade_no=%s user_id=%d client_ip=%s error=%q topup=%q", verifyInfo.ServiceTradeNo, topUp.UserId, c.ClientIP(), err.Error(), common.GetJsonString(topUp)))
			return
		}
