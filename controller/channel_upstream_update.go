...
func fetchAdvancedCustomUpstreamModelIDs(ctx context.Context, channel *model.Channel, baseURL string) ([]string, error) {
	...
	body, err := getResponseBodyWithContext(ctx, http.MethodGet, requestURL, channel, headers)
	if err != nil {
		return nil, sanitizeFetchModelsError(err, key)
	}
...
func StartChannelUpstreamModelUpdateTask() error {
	var startErr error
	channelUpstreamModelUpdateTaskOnce.Do(func() {
		...
		startErr = backgroundtask.Start("channel-upstream-model-update", func(ctx context.Context) {
