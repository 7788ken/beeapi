...
func UpdateMidjourneyTaskBulk(ctx context.Context) {
	billinglifecycle.RunPeriodic(ctx, 15*time.Second, false, func() {
		UpdateMidjourneyTaskOnce(context.WithoutCancel(ctx))
	})
}

// UpdateMidjourneyTaskOnce completes one polling round.
func UpdateMidjourneyTaskOnce(ctx context.Context) {
	...
	for channelId, taskIds := range taskChannelM {
		...
		if err != nil {
			cancel()
			logger.LogError(requestCtx, fmt.Sprintf("Get Task Do req error: %v", err))
			continue
		}
		if resp.StatusCode != http.StatusOK {
			...
			req.Body.Close()
			cancel()
			logger.LogError(requestCtx, fmt.Sprintf("Get Task status code: %d", resp.StatusCode))
			continue
		}
		...
		if err != nil {
			...
			req.Body.Close()
			cancel()
			logger.LogError(requestCtx, fmt.Sprintf("Get Mjp Task parse body error: %v", err))
			continue
		}
		...
		if err != nil {
			...
			req.Body.Close()
			cancel()
			logger.LogError(requestCtx, fmt.Sprintf("Get Mjp Task parse body error2: %v, body: %s", err, string(responseBody)))
			continue
		}
		resp.Body.Close()
		req.Body.Close()
