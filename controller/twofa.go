...
func Disable2FA(c *gin.Context) {
	if !middleware.RequireSecurityProof(c, "2fa.disable", []string{secureVerificationMethod2FA}) {
		return
	}
...
func Verify2FALogin(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "认证流程已失效，请重新登录",
		})
		return
	}
	if flow.Provider != "password" || flow.Intent != "login" || flow.UserID <= 0 {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "认证流程无效，请重新登录",
		})
		return
	}
	userId := flow.UserID
	...
	consumedFlow, err := service.ConsumeAuthFlow(
		req.FlowToken,
		service.AuthFlowPurposeTwoFA,
		time.Now(),
		nil,
	)
	if err != nil || consumedFlow.UserID != user.Id {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "认证流程已失效，请重新登录",
		})
