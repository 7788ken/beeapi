...
func RecomputeChannelMetrics(c *gin.Context) {
	err := backgroundtask.Submit("manual-channel-metrics", func(context.Context) {
		ctx, cancel := context.WithTimeout(context.Background(), 4*time.Minute)
		defer cancel()
		if err := service.RecomputeChannelMetricsOnce(ctx); err != nil {
			// 错误已在 service 层 logger 记录
			_ = err
		}
	})
	if err != nil {
...
func GetAllChannels(c *gin.Context) {
	...
	if enableTagMode {
		...
		var err error
		if c.Query("order_by") == "rpm_24h" {
			// RPM 排序特殊路径：rpm_24h DB 字段已废（值陈旧），显示值由 overlayRealtimeChannelRPM
			// 用 Redis 60s 滑动窗口覆盖。DB ORDER BY 会导致"排序按陈旧值、显示按实时值"不一致，
			// 改 Go 层按 Redis 值排序再翻页（同用户列表 listUsersOrderedByRealtimeRPM）。
			channelData, err = listChannelsOrderedByRealtimeRPM(baseQuery, pageInfo, c.Query("order"))
		} else {
			order := "priority desc"
			if idSort {
				order = "id desc"
			}
			if c.Query("order_by") == "used_quota" {
				// 「已使用 / 剩余」列排序口径 = used_quota（DB 实时列，与单元格左侧数字同源）。
				if c.Query("order") == "asc" {
					order = "used_quota asc, id desc"
				} else {
...
// listChannelsOrderedByRealtimeRPM 按 Redis 实时 RPM 排序后翻页（与用户列表 listUsersOrderedByRealtimeRPM 同模式）。
// 流程：baseQuery（已带 status/type 过滤）拿全部渠道 ID → Redis BatchGetChannelRPMs → 排序 → 切页 → DB 加载该页渠道。
// 渠道量级远小于 10k，全量 ID + RPM map 内存代价可忽略。order=desc 时 RPM 高的在前。
// 排序键优先级：RPM → 启用优先（禁用渠道 RPM 恒为 0，全挤在一起会淹没仍在用的 0 流量启用渠道）→ id desc。
func listChannelsOrderedByRealtimeRPM(baseQuery *gorm.DB, pageInfo *common.PageInfo, order string) ([]*model.Channel, error) {
	var rows []struct {
		Id     int
		Status int
	}
	if err := baseQuery.Select("id", "status").Scan(&rows).Error; err != nil {
		return nil, err
	}
	if len(rows) == 0 {
	...
	sort.SliceStable(ids, func(i, j int) bool {
		...
		// 同 RPM：启用渠道排在禁用渠道之前（升序降序都一样，禁用恒沉底）
		ei := statusByID[ids[i]] == common.ChannelStatusEnabled
		ej := statusByID[ids[j]] == common.ChannelStatusEnabled
		if ei != ej {
			return ei
		}
	...
	if err := model.DB.Where("id IN ?", pageIds).Omit("key").Find(&channels).Error; err != nil {
		return nil, err
	}

	// DB IN 查询返回顺序不一定与 pageIds 一致，重新按 pageIds 顺序排列
	chByID := make(map[int]*model.Channel, len(channels))
	for _, ch := range channels {
		chByID[ch.Id] = ch
	}
	ordered := make([]*model.Channel, 0, len(pageIds))
	for _, id := range pageIds {
		if ch, ok := chByID[id]; ok {
...
// overlayRealtimeChannelRPM 用 Redis 实时桶覆盖 RpmLast24h 字段（"最近 1 分钟实际请求数"）。
func overlayRealtimeChannelRPM(channels []*model.Channel) {
	if len(channels) == 0 {
		return
	}
...
// GetChannelKey 获取渠道密钥；路由层要求 channel.key.read Security Proof。
...
func validateChannel(channel *model.Channel, isAdd bool) error {
	...
	// cache_domain 列宽 varchar(64)：超长直接拒绝，避免落库时 DB 报 1406 裸错
	if channel.CacheDomain != nil && len([]rune(strings.TrimSpace(*channel.CacheDomain))) > 64 {
		return fmt.Errorf("缓存域长度不能超过 64 个字符")
	}

	// ratio_panel_url 列宽 varchar(255)：超长直接拒绝，避免落库时 DB 报 1406 裸错
	if channel.RatioPanelUrl != nil && len([]rune(strings.TrimSpace(*channel.RatioPanelUrl))) > 255 {
		return fmt.Errorf("倍率面板地址长度不能超过 255 个字符")
	}

	if channel.RatioUpstreamGroup != nil && len([]rune(strings.TrimSpace(*channel.RatioUpstreamGroup))) > 191 {
		return fmt.Errorf("上游分组名长度不能超过 191 个字符")
	}
...
func UpdateChannel(c *gin.Context) {
	...
	// 健康度接管：用户手工改 priority/weight → 清健康度快照（§6 兼容性）。
	// 顺序：必须在 channel.Update() 之后，避免被 Updates(struct) 覆盖。
	maybeResetChannelHealthOnManualEdit(c, originChannel, &channel.Channel)

	// 健康度接管：用户手工把 status 从非 Enabled 切到 Enabled → 清 Redis err_streak
	// + DB 健康度残留，避免"启用即死"。同样在 channel.Update() 之后。
	maybeResetChannelHealthOnManualEnable(c, originChannel, &channel.Channel)
...
func OllamaPullModelStream(c *gin.Context) {
	...
	key := strings.Split(channel.Key, "\n")[0]
	streamCtx := httplifecycle.StreamContext(c.Request.Context())
	...
	// 执行拉取
	err = ollama.PullOllamaModelStream(streamCtx, baseURL, key, req.ModelName, progressCallback)
