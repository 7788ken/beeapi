...
func RequestWaffoPancakeAmount(c *gin.Context) {
	...
	if !operation_setting.IsGroupAllowed(setting.WaffoPancakeAllowedGroups, group) {
...
// metadata 主路径已实测稳定（service.ResolveWaffoPancakeTradeNo），buyer email 改为
// 真邮箱优先以便 Waffo 把收据 / 通知发到用户邮箱；用户未绑邮箱时降级到伪邮箱，
// webhook 兜底反查链路仍能命中（waffo_pancake.go 的 BuyerEmail 兜底分支）。
func getWaffoPancakeBuyerEmail(user *model.User, tradeNo string) string {
	if user != nil && strings.TrimSpace(user.Email) != "" {
		return user.Email
	}
	if t := strings.TrimSpace(tradeNo); t != "" {
		return t + "@new-api.local"
	}
	if user != nil {
...
func RequestWaffoPancakePay(c *gin.Context) {
	...
	session, err := service.CreateWaffoPancakeCheckoutSession(c.Request.Context(), &service.WaffoPancakeCreateSessionParams{
		...
		BuyerEmail:       getWaffoPancakeBuyerEmail(user, tradeNo),
		SuccessURL:       getWaffoPancakeReturnURL(),
		ExpiresInSeconds: &expiresInSeconds,
		// metadata 通道：期望 Waffo webhook 原样回传，实测中。
		Metadata: map[string]string{"tradeNo": tradeNo},
	})
	if err != nil {
	...
	// 当前 Waffo createSession 响应不返回 orderId（实测为空），这里只在非空时回写，
	// 留作上游若日后补字段的即时兼容路径。
	if orderID := strings.TrimSpace(session.OrderID); orderID != "" {
		topUp.ProviderOrderID = orderID
		if err := topUp.Update(); err != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo Pancake 回写 ProviderOrderID 失败 user_id=%d trade_no=%s order_id=%s error=%q", id, tradeNo, orderID, err.Error()))
		}
	}

	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo Pancake 充值订单创建成功 user_id=%d trade_no=%s session_id=%s order_id=%s amount=%d money=%.2f", id, tradeNo, session.SessionID, session.OrderID, req.Amount, payMoney))
