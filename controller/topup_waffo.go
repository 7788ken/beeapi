...
func RequestWaffoAmount(c *gin.Context) {
	...
	if !operation_setting.IsGroupAllowed(setting.WaffoAllowedGroups, group) {
...
func RequestWaffoPay(c *gin.Context) {
	...
	successReturnUrl := paymentReturnPath("/console/topup?show_history=true&pay_success=true")
	failedReturnUrl := paymentReturnPath("/console/topup?show_history=true")
