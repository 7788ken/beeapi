...
type BodyStorage interface {
	...
	// IsDisk 是否是磁盘存储
	IsDisk() bool
	// NewReader 返回一个从头开始、游标独立的只读副本。
	// 每次调用返回的 reader 都有自己的游标，不与 storage 本身或其它副本共享
	// seek 状态，因此可以在主 body 已被读取（甚至正在读取）时安全重放。
	// 关闭返回的 reader 只释放该副本，不会释放 storage；storage 关闭之后
	// NewReader 返回 ErrStorageClosed，不会交出失效的 reader。
	NewReader() (io.ReadCloser, error)
}

// ReplayableBody 是可上报字节数、可重复读取的出站请求体。
// 用于让 net/http 无法推断的 ContentLength / GetBody 能从 body 自身取回，
// 同时不向 HTTP transport 暴露 io.Closer（storage 生命周期仍归调用方）。
...
func (m *memoryStorage) NewReader() (io.ReadCloser, error) {
	...
	// 在同一份不可变底层数组上新建 bytes.Reader：零拷贝拿到独立游标。
	// NopCloser 让 Close 成为空操作，storage 的生命周期仍归持有者。
...
func (d *diskStorage) NewReader() (io.ReadCloser, error) {
	...
	// 对同一个缓存文件另开一个 fd：零拷贝拿到独立游标。
	// 关闭返回的 reader 只关这个 fd，storage 仍持有主 fd 并负责删文件；
	// Close 时的 unlink 不影响已打开的 fd（inode 由 fd 保活）。
...
// ReaderOnly 把 BodyStorage 包成只暴露读取与重放能力的出站 body：
// 隐藏 io.Closer，避免 http.NewRequest 断言出 io.ReadCloser 后由 transport
// 提前关掉底层 storage；同时保留 Size/NewReader，让请求构造侧能补回
// net/http 无法从类型擦除 reader 推断出的 ContentLength 与 GetBody。
func ReaderOnly(storage BodyStorage) ReplayableBody {
