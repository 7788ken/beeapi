...
var (
	...
	maskIPPattern     = regexp.MustCompile(`\b(?:\d{1,3}\.){3}\d{1,3}(?::\d+)?\b`)
	// maskApiKeyPattern matches patterns like 'api_key:xxx' or "api_key:xxx" to mask the API key value.
	// 值字符集含 base64/url-safe 字符（+/=），避免 base64 形态的密钥只被部分打码而泄露尾部。
	maskApiKeyPattern = regexp.MustCompile(`(?i)\b((?:api[-_]?key|api[-_]?token|access[-_]?token|auth[-_]?token|refresh[-_]?token|secret[-_]?key|api[-_]?secret|authorization)\s*["']?\s*[:=]\s*["']?)([A-Za-z0-9._\-+/=]{6,})`)
	// maskAuthSchemePattern masks the credential following a "Bearer "/"Basic " scheme prefix
	maskAuthSchemePattern = regexp.MustCompile(`(?i)((?:bearer|basic)\s+)([A-Za-z0-9._\-+/=]{6,})`)
	// maskedHostPortPattern strips ports left dangling after a host has been masked
	maskedHostPortPattern = regexp.MustCompile(`(\*\*\*(?:\.[A-Za-z0-9*\-]+)*):\d+`)
)
...
func NormalizeBillingPreference(pref string) string {
	...
	case "subscription_first", "subscription_then_auto", "wallet_first", "subscription_only", "wallet_only":
...
func MaskSensitiveInfo(str string) string {
	...
	str = maskAuthSchemePattern.ReplaceAllString(str, "${1}***")
	str = maskApiKeyPattern.ReplaceAllString(str, "${1}***")
	str = maskedHostPortPattern.ReplaceAllString(str, "${1}")
