...
func UpsertPasskeyCredentialWithTx(tx *gorm.DB, credential *PasskeyCredential) error {
	...
	if tx == nil {
		common.SysLog("UpsertPasskeyCredential: nil transaction provided")
		return fmt.Errorf("Passkey 保存失败，请重试")
	}
...
func DeletePasskeyByUserID(userID int) error {
	if err := DB.Transaction(func(tx *gorm.DB) error {
		if err := DeletePasskeyByUserIDWithTx(tx, userID); err != nil {
			return err
		}
...
func DeletePasskeyByUserIDWithTx(tx *gorm.DB, userID int) error {
	if userID == 0 {
		common.SysLog("DeletePasskeyByUserID: empty user ID")
		return fmt.Errorf("删除失败，请重试")
	}
	if tx == nil {
		common.SysLog("DeletePasskeyByUserID: nil transaction provided")
		return fmt.Errorf("删除失败，请重试")
	}
	// 使用Unscoped()进行硬删除，避免唯一索引冲突
