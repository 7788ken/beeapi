...
// EnsureUserOAuthBindingWithTx creates the exact custom-provider binding or
// accepts the same existing binding. A different subject remains a conflict.
func EnsureUserOAuthBindingWithTx(tx *gorm.DB, binding *UserOAuthBinding) error {
	if tx == nil || binding == nil {
		return errors.New("OAuth binding transaction is required")
	}
	if binding.UserId == 0 {
		return errors.New("user ID is required")
	}
	...
	err := tx.Where("user_id = ? AND provider_id = ?", binding.UserId, binding.ProviderId).
		First(&existing).Error
	if err == nil {
		if existing.ProviderUserId == binding.ProviderUserId {
			return nil
		}
		return errors.New("this OAuth provider is already bound to another account")
	}
...
// DeleteUserOAuthBindingsByUserIdWithTx deletes all OAuth bindings for a user
func DeleteUserOAuthBindingsByUserIdWithTx(tx *gorm.DB, userId int) error {
