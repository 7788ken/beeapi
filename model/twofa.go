...
func (t *TwoFA) Delete() error {
	...
	return DB.Transaction(func(tx *gorm.DB) error {
		return t.deleteWithTx(tx)
	})
}

func (t *TwoFA) deleteWithTx(tx *gorm.DB) error {
	if t.Id == 0 {
		return errors.New("2FA记录ID不能为空")
	}
...
func DisableTwoFA(userId int) error {
	...
	if err := DB.Transaction(func(tx *gorm.DB) error {
		if err := twoFA.deleteWithTx(tx); err != nil {
			return err
		}
...
func (t *TwoFA) Enable() error {
	...
	if err := DB.Transaction(func(tx *gorm.DB) error {
		if err := tx.Model(&TwoFA{}).Where("id = ? AND user_id = ?", t.Id, t.UserId).
			Updates(map[string]any{
				"is_enabled":      true,
				"failed_attempts": 0,
				"locked_until":    nil,
