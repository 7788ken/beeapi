...
| `RELAY_IDLE_CONN_TIMEOUT` | Maximum idle keep-alive time for relay HTTP connections, in seconds; `0` disables the limit; restart required after changes | `90` (fixed default) |
| `STREAM_SCANNER_MAX_BUFFER_MB` | Max per-line buffer (MB) for the stream scanner; increase when upstream sends huge image/base64 payloads | `64` |
| `MAX_REQUEST_BODY_MB` | Max request body size (MB, counted **after decompression**; prevents huge requests/zip bombs from exhausting memory). Exceeding it returns `413` | `32` |
| `ANONYMOUS_REQUEST_BODY_LIMIT_KB` | Max body size (KiB) for anonymous API POST routes such as sign-in and payment callbacks; `0` disables the limit | `512` |
